# budget-code-mapper v[N+1] — Build Spec (Surgical Diffs)
...
**For the rendering agent.** Read the current source from `budget-code-mapper/source/` (base64-armored chunk set), then apply the diffs below. Ship as v[N+1].

## Source location

- **Chunk index:** `budget-code-mapper/source/source_index.html`
...
1. Read the chunk set from `budget-code-mapper/source/source_index.html`. Gated walk, auto-advance up to 5 chunks per pass.
