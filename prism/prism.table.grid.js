/* Prism — Table lens · GRID.
   ...
   editing, sort headers, column operations, the diff banner.
...
(function () {
  ...
  function patchCell(rid, k) {
    ...
    if (!td || !r) return;
    td.className = (TB.gate(r.c[k]) === "hex" ? "gate-hex" : "editable") + (TB.dirty(r, k) ? " has-dirty" : "");
  ...
  TBUI.grid = function () {
    ...
    T.cols.forEach(function (col) {
      ...
      h += '<th class="sortable" data-k="' + col.k + '"><span class="th-in">' +
    ...
    T.cols.forEach(function (col) {
      h += '<th class="cstat-cell" data-k="' + col.k + '"><span class="cstat">' + TB.colStat(col.k) + "</span></th>";
    });
    h += "</tr></thead><tbody>";
