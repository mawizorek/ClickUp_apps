/* chrome.js — standard app chrome, placed UNFORKED from template-app (PR #300).
   ⚠️ Byte-identical to `template-app/chrome.js` on purpose. Every app that copies this owns a FORK,
   so a fix here reaches no other app. Do not special-case this copy: everything app-specific must
   ride in through `cfg` from index.html. If you need something the template cannot express,
   change the TEMPLATE — that is Michael's call.
   ...
   All color comes from the theme spine via var(--token); no color literals here (sole exception:
   the neutral black-alpha scrim, which must darken regardless of theme). */
(function () {
