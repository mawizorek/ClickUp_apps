...
## 🎨 Adding a theme (START HERE)

**The whole job is: copy the template, fill 17 tokens, register one line, regenerate the CSS.** Four touches, then the preview menu auto-populates itself.

1. **Copy** `_template.json` → `<slug>.json` (e.g. `americana.json`). For an F1 constructor, put it in `f1/`.
2. **Fill it in:** set `name`, `slug`, `mode` (`light`|`dark`), `description`, and **all 17 tokens in OKLCH**. Every key must be a real color (no placeholders). Token roles are listed in `_index.json` → `tokenRoles`. Flip `status` to `locked`. Delete the template's `_README` key.
3. **Register one line** in `_index.json`: add `{ "slug", "name", "file", "status":"locked", "mode", "intendedUse", "fmp":"pending" }` under a group (or add a new group).
4. **Regenerate** `themes.css`: run `node build-themes.mjs`. This writes the `[data-theme="<slug>"]` block and schema-checks all 17 keys.

That's it. `preview.html` reads `_index.json` at load, so the new theme **appears in the menu automatically** with its swatch, no edit to the preview. Verify at `preview.html`: select it, confirm **17/17 tokens · all 20 objects styled**, eyeball each zone (see `OBJECT-COVERAGE.md`).

> **Why 4 steps and not 1:** styling + the menu auto-populate, but `themes.css` is a GENERATED artifact (the static, can't-white-screen styling source) and `_index.json` is the registry. Editing those two + the theme file is the contract; the build step keeps the CSS in lockstep. Never hand-edit `themes.css`.
...
```
`resolve.js` also sets `data-mode="light|dark"` and `data-theme="slug"` on the root, so mode-conditional CSS keys off the theme. It also carries a global light/dark toggle: `THEMES.setMode('light'|'dark')` flips every theme to its absolute mode (persisted in localStorage), degrading gracefully to a theme's default ramp when no opposite-mode ramp is authored. Static alternative: link `themes.css` and set `data-theme="slug"` on the root, no JS.
...
- **`resolve.js`** — shared slug→tokens resolver. Every hosted consumer calls it; single source of resolution truth. Also owns the global light/dark mode engine (`setMode`/`getMode`/`supportsMode`).
...
- **`preview.html` + `preview.css` + `preview.data.js` + `preview.core.js`** — the **theme × object gallery** (Theme Studio), split into the modular app layout (2026-07-17). `preview.html` is the slim boot shell; `preview.css` the styles; `preview.data.js` the embedded first-paint snapshot; `preview.core.js` the render engine + drawer chrome + live TSV/JSON override. Renders all 20 canonical objects from the active theme with a live token-coverage readout; its menu auto-populates from the canonical grids. Pick a slug to reskin every object at once; this is the reskin/compare tool AND the coverage proof.
...
shared/themes/
  ...
  decision-log.md      why the settled calls were made (read on enter, append on decide)
  _template.json       COPY-ME starter for a new theme (all 17 tokens + role hints)
  ...
  resolve.js           shared slug→tokens resolver (+ global light/dark mode engine)
  build-themes.mjs     generator (JSON → themes.css, schema check)
  preview.html         Theme Studio — slim boot shell (modular app layout)
  preview.css          Theme Studio styles
  preview.data.js      Theme Studio embedded first-paint snapshot (fallback data)
  preview.core.js      Theme Studio render engine + chrome + live TSV/JSON override
  maw-dark-utility.json  anchor theme + ultimate fallback
  paper-mono.json papyrus.json americana.json   (Neutral & Natural group)
...
| Neutral & Natural | `paper-mono` `papyrus` `americana` | light |
