...
**Status:** locked
**Supersedes:** the `color-mix(in oklch, ...)` gradient recipe + `grad-lift`/`grad-drop` feeling tokens; and "gradient stop colors baked to concrete values" is now hardened to "stored as explicit hex columns, never computed at all."

## 2026-07-17 · Colors + feelings stored as TSV grids; theme join stays JSON
**Decision:** The two defined-variable sets move to **tab-separated grids**: `colors.tsv` (one row per color, 17 hex columns + the extra gradient-stop column) and `feelings.tsv` (one row per feeling, the form columns). The **theme** stays JSON (`_themes.json`): it references a color row + a feeling row and holds the default fallback for any unlisted object. `resolve.js` gains a small TSV parser (fetch text → split rows → map columns to `--tokens`); consumers are unchanged (still read `var(--accent)`), and there is no color math to run since values are literal hex.
**Why:** Colors and feelings are uniform grids (every row = same columns), which per the repo CSV-vs-JSON gate belongs in a grid format, not JSON. The grid renders as a sortable table in GitHub AND makes a missing value a **visibly blank cell** — the grid itself becomes the completeness check (Michael's "force a more complete table"). **TSV not CSV:** hex is comma-safe, but feeling values are NOT (font stacks `'Chakra Petch', system-ui, ...` and shadow specs `0 1px 2px ..., 0 2px 6px ...` contain commas). Tabs appear in none of our values, so TSV is clean for both grids with one parser; CSV would force per-cell quoting on feelings. The join is ragged/nested (references + optional per-object overrides + growth room for broken-out font size/family later), which is the JSON shape, and JSON owns the default-if-unlisted fallback.
**Status:** locked
...
**Decision:** Depth via shadows and gradient fills is explicitly IN the themeable toolbox. FileMaker's Advanced Graphic area provides one outer + one inner shadow per object (color, opacity, blur, spread, offset) and Fill supports multi-stop gradients with an angle. The "infill rounded-rectangle shadow" = an inner shadow on a rounded rect (the pressed/inset button look). Budget is one outer + one inner per object (no stacked shadows). NOTE (updated same day): gradient stop colors are not just "baked" but stored as explicit hex columns — see the hex-canonical decision at top; no runtime color math at all.
