...
- **Phase 2 — Base Show File Build (Smith Theatre)** *(active)*: the accurate file itself; carries the rebuild-risk mitigation (D-008). Plan authored in git (D-018..D-022); `.vwx` follows.
...
- **Sheet-layer numbering scheme:** full Smith per-department list **DRAFTED** in `smith-theatre/standards/sheet-layers.md` (D-021); stays an F-016 DRAFT pending Michael's ruling to promote to a Standard.
...
- **Object-class tree (D-012/S-1):** the specific categories (steel / wood / framing / masking / …) + dash hierarchy. **PROPOSED** in `smith-theatre/standards/classes.md` (D-022); awaiting Michael's ruling before promotion + a `classes.csv`.
- **House layer list (D-012/S-1):** the department × elevation set is **AUTHORED as a working draft** in `smith-theatre/standards/layers.csv` (D-019). Lock + promote to Standard on Michael's ruling; resolve the `CUT?`/import candidates.
...
- **Existing sheet-layer scheme (ClickUp):** dept-prefixed numbered drawings — UR0–6 (file readme, groundplan, overhead, W/N/E/S sections), S1–7 (scenic), L0– (lighting), A0– (audio), R0– (rigging), V0– (video); indented sheet number = a viewport off the sheet above. Basis for the F-016 draft numbering (promoted to the Smith per-dept draft, D-021).
- **Layer worksheet (working draft):** Google Sheet "URITP VWX Smith Theatre BASE FILE Worksheets" — ~27 layers keyed on DEPARTMENT × elevation band, with 2D/3D flags and STATUS. Authored into `smith-theatre/standards/layers.csv` (D-019).
- **Venue geometry (feeds S-3 / S-4):** blackbox rectangle ~50'×70' nominal; interior trim shaves ~1/8" per wall; deck measurements off interior trim face; mezzanine/catwalks off nominal wall structure. Center of rectangle = internal origin (built). Rule recorded in Smith `datums-and-reference-planes.md` (D-020); numbers stay in the file (S-4).
...
1. **Michael's rulings on the Smith open candidates** (ranked): object-class tree (D-022) → promote + author `classes.csv`; sheet-numbering (D-021) → promote F-016 to a Standard; lock the house layer list (D-019) → promote from working draft.
...
*Last updated: 2026-07-16 (D-018..D-022: Smith Theatre clone + per-instance fills).*
