...
>
> **Handoff rule:** if the working session is interrupted, another agent should be able to read this file top-to-bottom and resume the brainstorm with full context. Keep it current. New decisions **prepend** to the top of the Decisions Log (newest first, per the repo-wide log-ordering standard in `brain-config/team-standard.md`); open items live in Open Questions.
>
...
> **Ordering: newest first.** New decisions are **prepended** at the top (repo-wide log standard — `brain-config/team-standard.md`). Read top-to-bottom = most recent → oldest.

| ID | Date | Decision | Notes |
|----|------|----------|-------|
| D-018 | 2026-07-16 | **Adopt newest-first ordering for every ongoing log in the repo** (decision logs + changelogs): new entries **prepend** at the top so the most recent decision is always first. Flipped this Decisions Log (was D-001→D-017 oldest-first) to newest-first and switched the handoff rule from "append" to "prepend." | Codified repo-wide in `brain-config/team-standard.md` (v1.2) + noted in VWX-BEST-PRACTICES.md. Already-conforming logs (`filemaker/DECISIONS.md`, `brain-config/CHANGELOG.md`, `_TEMPLATE/CHANGELOG.md`, FileMaker `DOCUMENTATION-STANDARD.md`) left unchanged. |
