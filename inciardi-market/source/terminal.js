...
async function boot(){
 ...
 CATALOG = catalog && catalog.prints && catalog.prints.length ? catalog : await apiGetLocalCatalog();
 INV = inv;
...
function renderPortfolio(){
 ...
 $("pstats").innerHTML = `
  ...
  <div class="c stat"><div class="v ${plCls}">${pl==null?"\u2014":(pl>=0?"+":"")+money0(Math.abs(pl))}</div><div class="l">Unrealized P/L</div><div class="sub">cost ${cost?money0(cost):"\u2014"}</div></div>
  <div class="c stat"><div class="v ${realized>=0?"up":"down"}">${sold.length?((realized>=0?"+":"")+money0(Math.abs(realized))):"\u2014"}</div><div class="l">Realized</div><div class="sub">${sold.length} sold</div></div>`;
...
function renderDeals(){
 ...
 $("deals").innerHTML = deals.slice(0,5).map(({p,m,r})=>{
  const th = p.image? `<img src="${esc(proxied(p.image,88))}" alt="">` : `<div style="background:var(--surface-2);width:100%;height:100%;display:grid;place-items:center;color:var(--ink-faint)">${initials(p.name)}</div>`;
