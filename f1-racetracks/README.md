...
**Status:** v5 live · properly demonolithized runtime structure · source surface synced and judged · winners history backfilled through Monza · live tracker slice shipped
...
- the main app runtime is now loaded from modular files in `/source`, with `index.html` reduced to a slim fireable entrypoint
...
- A first live-tracking companion page that follows the latest OpenF1 session without destabilizing the main circuit experience
...
- **Runtime structure:** the app is no longer a giant monolithic runtime file. `index.html` is now a slim entrypoint that loads runtime styles and JS directly from `/source` plus data from `data.json`.
- **Spot-edit workflow:** use `/source` first. The runtime entrypoint is no longer the primary editing surface.
- **Completed-race panels:** podium / pole / history all render from the `raceResults` and `historicWinners` objects in `data.json`.
- **Live-tracking slice:** `live-tracker.html` is the first OpenF1-backed feature. It intentionally shipped as a companion page first so live tracking could go out immediately while the main app structure was cleaned up.
...
- decide whether the next OpenF1 step folds the live slice into the main circuit experience or expands the companion surface first
