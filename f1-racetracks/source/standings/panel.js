...
function seasonBrief(id){
  ...
  trajState={id,metric:'standings'};
  const rows=ROUNDS.map(rd=>{const r=raceRow(rd,id);const sp=rd.sprint?rd.sprint.classification.find(x=>x.driverId===id):null;let fin='\u2014',pts=0;if(r){fin=r.status==='DNF'?'DNF':'P'+r.pos;pts=r.points+(sp?sp.points:0);}return `<div class="season-row"><span class="sr-rn num">R${rd.round}</span><span class="sr-ev">${codeOf(rd)} \u00B7 ${rd.name.replace(' Grand Prix','')}</span><span class="sr-fin num" style="color:${r&&r.pos===1?'var(--gold)':r&&r.status==='DNF'?'var(--red)':'var(--txt)'}">${fin}</span><span class="sr-pts num">${pts||''}</span></div>`;}).join('');
