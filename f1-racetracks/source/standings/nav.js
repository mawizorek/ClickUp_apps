...
.circuit-link{display:flex;align-items:center;justify-content:space-between;gap:10px;margin-top:2px;padding:12px 14px;border:1px solid var(--line);background:var(--s1);text-decoration:none;transition:background .15s var(--ease)}
...
`;
  ...
  function fill(){
    ...
    menu.innerHTML=rs.length?rs.map(rd=>'<a role="menuitem" href="'+CIRCUITS+'#/'+rd.slug+'"><span class="rn">R'+rd.round+'</span><span class="rc">'+shortName(rd.name)+'</span></a>').join(''):'<a href="'+CIRCUITS+'"><span class="rc">Open circuits &#8594;</span></a>';
  }
  ...
  // race brief -> circuit guide connector (runs after panel.js has set the panel HTML)
