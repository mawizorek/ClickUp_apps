...
function renderPole(result) {
  const pole = result && result.pole;
  if (!pole) return "";

  return `
    <article class="pole-panel">
      <div class="pole-kicker">
        <div>
          <div class="tag">Pole position</div>
          <h3>Qualifying snapshot</h3>
        </div>
        <div class="tag">Saturday pace</div>
      </div>
      <div class="pole-big">${esc(lastName(pole.driver))}<small>${esc(pole.team || "")}</small></div>
      <div class="pole-meta">
        <div class="pole-chip"><span class="k">Pole time</span><span class="v">${esc(pole.time || "TBC")}</span></div>
        <div class="pole-chip"><span class="k">Gap to P2</span><span class="v">${esc(pole.gapToP2 || "TBC")}</span></div>
      </div>
      ${Array.isArray(pole.top5) && pole.top5.length ? `
        <table class="quali-mini">
          <thead><tr><th>Driver</th><th>Time</th></tr></thead>
          <tbody>
            ${pole.top5.map(row => `
              <tr>
                <td>${esc(lastName(row.driver))}<div style="font-size:.72rem;color:${teamTone(row.team)};margin-top:2px">${esc(row.team)}</div></td>
                <td>${esc(row.time)}</td>
              </tr>
            `).join("")}
          </tbody>
...
function renderHistory(slug) {
  const rows = historicWinners[slug] || [];
  return `
    <section class="block">
      <div class="history-board">
        <div class="history-head">
          <div>
            <div class="tag">Race history</div>
            <h2>Winners</h2>
          </div>
          <div class="tag">Most recent first</div>
        </div>
...
function renderTrack(t) {
  ...
  const prev = reportTracks[idx - 1];
  const next = reportTracks[idx + 1];
  const vals = t.profile.map(p => p[1]);
  const aslLow = Math.round(t.asl0 + Math.min(...vals));
  const aslHigh = Math.round(t.asl0 + Math.max(...vals));
  const aslSF = Math.round(t.asl0 + t.profile[0][1]);
  const result = raceResults[t.slug] || null;
  const featureMarkup = result && (renderPodium(result) || renderPole(result))
    ? `<section class="feature-grid">${renderPodium(result)}${renderPole(result)}</section>`
    : "";
  ...
  app.innerHTML = `
    <div class="view">
      ...
      <div class="card">
        ...
        <div class="plate">
          <img loading="lazy" src="https://commons.wikimedia.org/wiki/Special:FilePath/${encodeURIComponent(t.mapFile)}" alt="Official ${esc(t.title)} outline map" onerror="this.style.display='none';this.insertAdjacentHTML('afterend','<div class=&quot;ph&quot;>Map image unavailable yet. File: ${esc(t.mapFile)}</div>')">
        </div>
      ...
      <div class="info2">
        <div class="panel">
          <div class="panel-h"><i data-lucide="circle-gauge"></i><h3>Pit lane & stops</h3></div>
          <div class="panel-b">
            ${t.pit.map(p => `<div class="kv"><span class="k">${esc(p[0])}</span><span class="v">${esc(p[1])}${p[2] ? `<small>${esc(p[2])}</small>` : ""}</span></div>`).join("")}
          </div>
        </div>

        <div class="panel">
          <div class="panel-h"><i data-lucide="disc"></i><h3>Tyre strategy</h3></div>
          <div class="panel-b">
            <div class="tyres">
        ...
        <div class="panel">
          <div class="panel-h"><i data-lucide="swords"></i><h3>Overtaking</h3></div>
          <div class="panel-b">
            <div class="rate">
              <div class="score">${typeof t.otScore === "number" ? t.otScore.toFixed(1) : t.otScore}<small> / 5</small></div>
              <div class="meter"><i style="width:${t.otPct}%"></i></div>
              <div class="word">${esc(t.otWord)}</div>
            </div>
            <div class="spots">${t.spots.map(s => `<div class="s"><b>${esc(s[0])}</b><span>${esc(s[1])}</span></div>`).join("")}</div>
            <p class="note">${esc(t.otNote)}</p>
          </div>
        </div>
      </div>
      ...
      <section class="block">
        <div class="h2"><h2>Corner-by-corner</h2><span class="count">${t.turns} turns</span></div>
        <table class="tbl">
          <thead><tr><th>#</th><th>Name</th><th>Sector</th><th>Type</th><th>Notes</th></tr></thead>
          <tbody id="tbody"></tbody>
        </table>
      </section>

      <section class="block">
        <div class="h2"><h2>Track character & geography</h2></div>
        <div class="geo">${t.geo.map(g => `<article><h3><i data-lucide="${g[0]}"></i>${esc(g[1])}</h3><p>${esc(g[2])}</p></article>`).join("")}</div>
      </section>
...
function buildProfile(t) {
  ...
  const X = d => PAD + d * (W - PAD * 2);
  const Y = m => EB - ((m - elevMin) / (elevMax - elevMin)) * (EB - ET);
  ...
  const defs = E("defs");
  const grad = E("linearGradient", { id: "eg", x1: 0, y1: 0, x2: 0, y2: 1 });
  grad.appendChild(E("stop", { offset: "0%", "stop-color": "var(--elev)", "stop-opacity": .32 }));
  grad.appendChild(E("stop", { offset: "100%", "stop-color": "var(--elev)", "stop-opacity": .02 }));
  ...
  svg.appendChild(E("path", { d: area(t.profile), fill: "url(#eg)" }));
  svg.appendChild(E("path", { d: line(t.profile), fill: "none", stroke: "var(--elev)", "stroke-width": 2.5, "stroke-linejoin": "round" }));
  ...
  if (Math.abs(ySF - yLo) > 10 && Math.abs(ySF - yHi) > 10) {
    ...
    const sl = E("text", { x: W - PAD, y: ySF - 5, fill: "var(--muted)", "font-size": 9.5, "text-anchor": "end", "font-weight": 600, "font-family": "IBM Plex Mono, monospace" });
    sl.textContent = "S/F " + asl(sfVal) + " m";
    svg.appendChild(sl);
  }

  t.drs.forEach(z => {
    const dx = X(z.det), ay = EB + 20;
    svg.appendChild(E("line", { x1: dx, y1: ay, x2: X(z.from), y2: ay, stroke: "var(--drs)", "stroke-width": 1.4, "stroke-dasharray": "3 3" }));
  ...
  t.sectorsBar.forEach(s => {
    svg.appendChild(E("rect", { x: X(s.from), y: BARY, width: X(s.to) - X(s.from) - 2, height: BARH, fill: SECraw(s.k), rx: 3 }));
  ...
  t.marks.forEach(m => {
    const x = X(m[0]);
    svg.appendChild(E("line", { x1: x, y1: Y(valAt(m[0])), x2: x, y2: BARY, stroke: "var(--line)", "stroke-width": 1, "stroke-dasharray": "2 4" }));
    svg.appendChild(E("circle", { cx: x, cy: Y(valAt(m[0])), r: 3, fill: "var(--elev)", stroke: "var(--bg)", "stroke-width": 1.5 }));
    svg.appendChild(E("text", { x, y: Y(valAt(m[0])) - 9, fill: "var(--muted)", "font-size": 10, "text-anchor": "middle", "font-family": "Space Grotesk, sans-serif" })).textContent = m[1];
  });
}
