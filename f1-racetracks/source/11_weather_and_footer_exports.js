...
const weekendCenterState = {};

function ensureWeekendState(slug) {
  if (!weekendCenterState[slug]) {
    weekendCenterState[slug] = { mode: null, beat: 0, speed: 1, timer: null };
  }
  return weekendCenterState[slug];
}
  ...
  (track.sessions || []).forEach((entry) => {
    ...
    grouped.push({
      ...
      items: pieces.map((label) => ({
        series: label.startsWith("Sprint") || label === "Qualifying" || label === "Practice" ? "F1" : "Weekend",
        label,
        time: "Time TBC",
...
function scheduleDataFor(track) {
  if (Array.isArray(track.schedule) && track.schedule.length) {
    ...
    track.schedule.forEach((item) => {
      ...
      if (!grouped[dateKey]) {
        grouped[dateKey] = { day: new Date(item.datetime).toLocaleDateString("en-US", { weekday: "short" }), date: dateKey, items: [] };
      }
...
function renderScheduleMode(track) {
  ...
  const nextUp = groups.flatMap((group) => group.items).find((item) => item.status === "upcoming") || null;
  return `
    <div class="wc-body-grid">
      ...
      <aside class="wc-rail">
        <div class="wc-rail-card">
          ...
          <div class="wc-rail-value">${nextUp ? esc(nextUp.label) : "No upcoming session queued"}</div>
        </div>
        <div class="wc-rail-card">
          <div class="wc-rail-label">Intent</div>
          <div class="wc-rail-copy">As you page through rounds, the app should feel like a running calendar.</div>
        </div>
      </aside>
    </div>
  `;
...
function mountWeekendCenter(explicitSlug) {
  const slug = explicitSlug || trackSlugFromHash();
...
window.addEventListener('hashchange', () => {
  ...
  Object.keys(weekendCenterState).forEach(clearWeekendReplayTimer);
  window.setTimeout(() => mountWeekendCenter(slug), 0);
});

window.setTimeout(() => mountWeekendCenter(trackSlugFromHash()), 400);