...
const RETIRED_KEY = 'appDashboard_showRetired';
...
const SKELETON = '<div class="skeleton">' +
 ...
 '<div class="sk-row"><div class="sk-tile"></div><div class="sk-body"><div class="sk-bar" style="width:50%"></div><div class="sk-bar" style="width:62%;height:8px"></div></div></div>' +
 '</div>';
...
function applyFilter(filter, persist){
 ...
 document.querySelectorAll('.chip[data-filter]').forEach(function(c){ c.classList.toggle('on', c.dataset.filter===filter); });
 const rows = document.querySelectorAll('.row[data-category]');
 let visible = 0;
 rows.forEach(function(r){
 const catOk = filter==='all' || r.dataset.category===filter;
 const retiredOk = showRetired || r.dataset.status!=='retired';
 const show = catOk && retiredOk;
 r.classList.toggle('hide', !show); if(show) visible++;
 });
...
function setShowRetired(on){
 showRetired = on;
 try{ localStorage.setItem(RETIRED_KEY, on?'1':'0'); }catch(e){}
 const t = document.getElementById('retiredToggle');
...
async function init(force){
 ...
 try{ showRetired = localStorage.getItem(RETIRED_KEY)==='1'; }catch(e){}
 const rt=document.getElementById('retiredToggle'); if(rt){ rt.classList.toggle('on', showRetired); rt.setAttribute('aria-pressed', showRetired?'true':'false'); }
...
async function buildApp(path, slug, hasData, hasIcon){
 ...
 status:(meta.status||'live'), supersededBy:(meta.supersededBy||null),
...
document.getElementById('filterRow').addEventListener('click', function(e){
 const rt = e.target.closest('#retiredToggle');
 if(rt){ setShowRetired(!showRetired); return; }
 const chip=e.target.closest('.chip[data-filter]'); if(!chip) return; applyFilter(chip.dataset.filter);
});
