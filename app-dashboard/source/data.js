/* app-dashboard — data module. APP_META + SAMPLE (ClickUp window) and FM_META + FM_SAMPLE (FileMaker window).
 ...
 NOTE on `supersededBy`: it names another APP slug. Leave it off when an app was replaced by
 something that isn't an app (a doc, an agent) — say so in `desc` instead, or the launcher links
 to a tile that doesn't exist. */
...
const APP_META = {
 ...
 'routines': { desc: 'Deleted Jul 2026. The routine schedule now lives only in routines/schedule.md, read directly or via Routine Ricky. No app.', category: 'dashboard', badges: ['dashboard'], status: 'retired' },
...
const SAMPLE = [
 ...
 { name:'routines', hoursAgo:1, commitCount:10, commits:[['DELETED — schedule.md is the single source',1],['v2: invoke-only header + real last-run stamps',3],['add passive live schedule viewer',40]] },
