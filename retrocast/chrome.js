  ...
  function buildSettingsDrawer(cfg) {
    ...
    d.innerHTML =
      ...
      '<div class="drawer-body">' +
        ...
        '<p class="drawer-note">Retrocast reads a stored data table, refreshed daily per location and on open, with a manual refresh button. Today comes from the forecast model; history from the ERA5 archive (~5-day lag). Weather data: Open-Meteo (CC BY 4.0). On this day: Wikimedia. Holidays: Nager.Date. All times anchor to each location&#39;s local date.</p>' +
      '</div>';
    document.body.appendChild(d);
