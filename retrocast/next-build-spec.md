# Retrocast — v1 Build Spec

> **Name LOCKED: Retrocast** (Michael, 2026-07-19 — a “forecast” pointed backward, past-vs-present, not weather-boxed). Folder slug `retrocast`. This file is the SINGLE SOURCE OF TRUTH for the v1 render handoff.
>
...
>
> **Provenance:** brainstormed 2026-07-18 with the full Council + Workshop. Decision Log: `Weather Almanac — Decision Log` (ClickUp; title predates the rename, content current). Session task: Agent Activity Board `Brain (Opus 4.8) · Weather Almanac … brainstorm · Jul 18`. APPS project task: `Retrocast — forecast-vs-history viewer app`.
...
| # | Decision | Detail |
|---|---|---|
| Name | **Retrocast** | Locked 2026-07-19. Slug `retrocast`. |
...
- **Weather baseline = TSV** (uniform grid). One file per location: `retrocast/data/<location-slug>/baseline.tsv`.
...
- A separate helper (a Routine-Ricky-style data job or a small build script) refreshes the repo TSV/JSON on a schedule. The APP never writes repo files; it only reads them + its own cache. This is Michael's “live fetch into our current data table, reference our own stored data, not hardcoded external calls” instinct. Spec the helper as `retrocast/routines/almanac-refresh.md` (agent-agnostic runbook) in a later pass; v1 render does NOT need it built — it can seed the 3 locations' baselines once and rely on cache for the rest.
...
8. **Footer** — standard stamp `Retrocast v1 · PR #<n>` (from `chrome.js`).
...
1. Read THIS file end to end. Read the linked Decision Log (all Q blocks + S1/S2/S3/S4 synthesis) and the session task's Workshop comments for rationale.
...
6. Name is LOCKED = Retrocast (slug `retrocast`).
