...
| `inbox-digest-report` | v3.1 (shell) · data: static seed 2026-07-16 | live | Data-audit surface for the Gmail Inbox Sweep. **v3.1 (PR #318): bugfix — slop clear query scoped to `in:inbox`.** A bare `from:(...)` searches ALL mail (archive + every label); the Jul 16 sweep's unscoped query surfaced each sender's full history (archived newsletters, ACCOUNTS-labeled Lightwright license records, a Payment Confirmation w/ attachment), none of them the 3 inbox slop threads — select-all+delete would've nuked keepers. Fix: `in:inbox from:(...)`, now locked into the report format. Only `pages/report.html` slop block + `decision-log.md` changed; shell v3→v3.1. **v3 (PR #316): added `pages/report.html` (Gmail Clean-into-ClickUp Pipeline Report) and made it the LANDING page.** Five locked buckets (TO_MERGE / NEEDS_REPLY / HAS_ATTACHMENTS / ALREADY_HANDLED / SLOP, every header always renders), inline live gmail_url links + destination task links, grouped slop + one-click clear block, inbox-zero scoreboard, two-phase PLAN/EXECUTE spine with a per-row status pill; Jul 16 sweep (same 5 threads as matrix) hardcoded. `index.html`: `DEFAULT_PAGE` matrix→report, NAV = Report + Field Matrix + About. `pages/matrix.html` UNTOUCHED (byte-for-byte), now page 2. No Gmail/Action auth (locked). `data/inbox-state.json` fetch = next build. **v2 (PR #312): rebuilt from the gold-standard `template-app`** - real chrome (header, LEFT nav drawer, RIGHT settings drawer w/ live theme picker, footer stamp) + `styles.css` (template verbatim + `.matrix-*` grid block) + `pages/` partials; `pages/matrix.html` = field-capture matrix (10 captured fields, live Jul 16 sweep rows); `pages/about.html`. maw-dark-utility default, swappable off the spine. v1 (PR #311) was an improvised shell, superseded. |
