...
| `f1-racetracks` | v10 (verified) | live | Unified single-app shell (`index.html` = REAL app: one persistent chrome + ONE hash router `source/app-shell.js`; each lens an isolated always-warm cross-faded surface, no reload). `#matrix`/`#history` → standings surface; `#circuits`/`#circuits/<slug>` → circuits surface; shell ADOPTS each lens on load, zero edits to the 24 lens modules, both still work standalone. v8: circuits database cards + Jump side drawer. v9: culled circuits hero + top-level filter bar (Continent/Era/Status/Breakdown; `source/09`). **v10 (PR #180): History lens restyled into a social/activity FEED (`source/standings/history.js`) — dropped the banned `.hx-rail` side-stripe for a timeline (connector line + team-tinted node) with full-bordered post cards, relative timestamps, podium body, championship-swing footer; pure restyle. Dead-CSS sweep in `02css`: removed orphaned `button.race`* + `.status-stripe`/`.ss-*` (banned side-stripe) + `.legend`/`.lg` (old home legend, now filter chips); left `.badge`/`.b-report`/`.b-soon` (generic-named, unverified across weekend modules). `standings.html` `?v` 6.5→6.6, footer → v6.6.** Team-home-race filter still deliberately NOT built (needs curated mapping). Verified via git blob API. |
