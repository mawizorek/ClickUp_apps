...
| `inbox-digest-report` | v3.2 (shell) · data: static seed 2026-07-16 | live | Data-audit surface for the Gmail Inbox Sweep. **v3.2 (PR #319): spreadsheet-dashboard skin for `pages/report.html`, mobile-primary.** Each bucket is now a titled grid block (mono header bar + count connected to its table), sharp corners, connected gridlines, dense mono labels. Fixed the mobile wrap bug (`table-layout:fixed` + column widths + `overflow-wrap:anywhere` — senders no longer shred one word per line). Pills/chips/tags squared on `.report`; status pills fill their cell as spreadsheet flags; slop-clear is a tap-to-select mono line. New CSS = `REPORT ADDITIONS` block appended to `styles.css` (matches the `MATRIX ADDITIONS` pattern; no local `:root`, all `var(--token)`); `index.html` styles.css cache token → `?v=3`. Matrix page + matrix CSS UNTOUCHED. No DESIGN-UI ban hits (full borders, no gradient text/glass/pure-black-white). **v3.1 (PR #318): bugfix — slop clear query scoped to `in:inbox`** (a bare `from:(...)` searched ALL mail and surfaced archived + ACCOUNTS-labeled keepers + a receipt; select-all+delete would've nuked them). Now locked into the report format. **v3 (PR #316): added `pages/report.html` (Gmail Clean-into-ClickUp Pipeline Report) and made it the LANDING page.** Five locked buckets (TO_MERGE / NEEDS_REPLY / HAS_ATTACHMENTS / ALREADY_HANDLED / SLOP, every header always renders), inline live gmail_url links + destination task links, grouped slop + one-click clear block, inbox-zero scoreboard, two-phase PLAN/EXECUTE spine with a per-row status pill; Jul 16 sweep (same 5 threads as matrix) hardcoded. `DEFAULT_PAGE` matrix→report, NAV = Report + Field Matrix + About. `data/inbox-state.json` fetch = next build. **v2 (PR #312): rebuilt from the gold-standard `template-app`** - real chrome + `styles.css` (template verbatim + `.matrix-*` grid block) + `pages/` partials; `pages/matrix.html` = field-capture matrix; `pages/about.html`. maw-dark-utility default. v1 (PR #311) was an improvised shell, superseded. |
