...
| `routines` | **v2** · 2026-07-27 | PR #560. Passive viewer of `routines/schedule.md` + the per-routine `last-run/*.txt` stamps. **INVOKE-ONLY** (scheduler retired 2026-07-26): the header card states the MODEL and **derives** every count — **never put a time of day in it**, that is exactly how v1 ended up printing a wake schedule after nothing woke. v2 closed 6 defects, incl. **every routine silently reading "Last run: never" for 3 weeks** (col 4 became a path on 07-05; the parser kept date-parsing it). `never` / unreadable / overdue stay visibly distinct **by rule**. ⚠️ **~20.8KB — over the 15KB split line; next change is the `source/` split, not another inline block.** ⚠️ **This app and `schedule.md` are ONE SYSTEM — reshaping that table is an app change.** Spec + full defect history: `routines/next-build-spec.md`. |
...
- **2026-07-27 — `routines` stamped v2** (PR #560), closing the **last `no version stamp` row** in the ledger. Worth recording *why* it mattered: that app had been rendering "Last run: never" for every routine since 2026-07-05 and quoting a wake schedule for a scheduler that no longer exists — **an unversioned app is an unverified app**, and both defects sat in the one surface whose entire job was reporting status. Also flagged on the `world-cup-bracket` row that its ROUTINE retired while the APP stayed live.
