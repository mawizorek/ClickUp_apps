...
| `inciardi-market` | shell v12 (verified) | live | Multi-page collector's terminal (terminal/catalog/collection/market + thin index router). v12 (PR #247, merge `3eb76fe`): (1) **catalog.html is now the default landing** (`index.html` DEFAULT_LANDING); (2) **mobile slide-out nav drawer** — below 640px the inline nav is replaced by a hamburger + right-side drawer built in `app-core.js buildMobileNav()` (full open/close lifecycle: backdrop/X/Esc/re-tap/link-tap, body scroll lock, reduced-motion); (3) **DATA FIX** — `apiGet`/`apiPost` got a 6s AbortController timeout; the Cloudflare Worker is currently unreachable and the old code awaited it with no timeout, so `boot()` hung on “connecting…” and never fell back to the `catalog.json` seed. Now a dead Worker fails fast and the seed (~50 prints) renders. Also fixed footer BUILD stamp (was hardcoded v10.1 in app-core, overriding HTML). NOTE: Worker being down is separate infra to revive if live eBay/market data is wanted. |
...
_Rollout note: `unverified` rows carry the last value I could see without a fresh read; each gets confirmed and firmed up the next time that app is touched. Confirmed rows (prism, app-dashboard, markdown-viewer, world-cup-bracket, on-track, filemaker, inciardi-market) are current as of their listed dates._
