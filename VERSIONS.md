...
| `inbox-digest-report` | v5 (shell) · data: Jul 17 2026 sweep | live | Data-audit surface for the Gmail Inbox Sweep. **v5 (PR #322): report reskinned as a mobile-primary VIEWER.** Each email renders as a stacked ROW UNIT (status pill / sender+subject / plan) instead of a `<table>`; at `>=720px` the same DOM reflows into 3 aligned columns (reads like a table, isn't one). Status strip 2x2→row-of-4; headline scoreboard number; calmer surface-bordered framing (dropped the squared spreadsheet look). Renderer + `styles.css` REPORT ADDITIONS block only; `index.html` styles.css token → `?v=4`, shell v4→v5. Matrix (drill-in) stays device-specific + untouched. Still a PURE RENDERER — JSON + data-only lock intact. **v4 (PR #321): app became a PURE RENDERER of `data/inbox-state.json`** (a sweep edits ONLY that JSON; `data/README.md` = field spec; router re-executes page scripts). **v3.2 (PR #319): spreadsheet-dashboard skin** (fixed mobile wrap; `REPORT ADDITIONS` block). **v3.1 (PR #318): slop clear scoped to `in:inbox`.** **v3 (PR #316): added `pages/report.html` as LANDING page** (five buckets, inline gmail_url + task links, grouped slop + one-click clear, scoreboard, PLAN/EXECUTE pill; `DEFAULT_PAGE` matrix→report). **v2 (PR #312): rebuilt from gold-standard `template-app`.** v1 (PR #311) improvised shell, superseded. |
