...
| `f1-racetracks` | v9 (verified) | live | Unified single-app shell (`index.html` = REAL app: one persistent chrome + ONE hash router `source/app-shell.js`; each lens an isolated always-warm cross-faded surface, no reload). `#matrix`/`#history` → standings surface; `#circuits`/`#circuits/<slug>` → circuits surface; shell ADOPTS each lens on load (chrome-hiding CSS + inner-hash drive), zero edits to the 24 lens modules, both still work standalone. v8: circuits database cards (`.rcard`/`.rc-*`) + Jump-to-circuit side drawer. **v9 (PR #178): culled the circuits marketing hero (H1 + paragraph gone; slim eyebrow + carousel kept; legend folded into Status filter chips) and added a real top-level FILTER BAR (`.filters`/`.chip`/`.flt-*`) — Continent (derived from `cc`), Era (Classic/Modern editorial split), Status, Breakdown; AND across / OR within, live count + reset + empty state; all self-contained in `source/09`. Footer culled in `circuits.html` to version meta + a single Live tracker link (dropped Championship Matrix dup + Copy source / Prepare download / Open in new tab / Export data). Cache token → 20260713e.** Team-home-race deliberately NOT built (ambiguous — needs a curated mapping). Old `.race`/`.status-stripe` in 02css still dead (sweep pending). Prior: v6.3–v6.5, v7 shell. Verified via git blob API. History-as-feed = next pass. |
