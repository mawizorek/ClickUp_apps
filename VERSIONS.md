...
| `world-cup-bracket` | shell v5.7 · data 2026-07-09 (verified) | live | Data-separated app: refreshes change only `data.json` (NO shell/version bump). **v5.7 (PR #122) — local-timezone kickoff:** `localKickoff` in `util.js` renders each kickoff in the VIEWER'S own zone with a short label (e.g. `4:00 PM CDT`), built from the true UTC instant; schedule cards + detail-sheet When line use it. Data `time` stays ET-clock (Ricky writes ET, app converts). **v5.6 (PR #119) — 3rd-place rendering:** round `3P` (id 32, Sat Jul 18) added to the QF+ schedule filter + a standalone "3rd Place" node under the Final (NOT wired into the championship tree — its teams are the SF losers). **v5.5 (PR #106) — app-derived status:** the app OWNS status. `normalizeMatches` in `util.js` derives upcoming/live/done at load (scores present → ft/`aet`/`pso`; kickoff passed & within window → live; else upcoming), so `data.json` carries FACTS only (scores, `psoNote`, `aet`, ET `time`, teams, `feedsTo`, `cuTaskId`) and NO status string. **Mirror keying (PR #110):** `cuTaskId` on all 32 matches (data.json match ↔ ClickUp World Cup list task, the ONLY join key); Ricky runbook + ops guide locked to facts-only + team-reveal rename (PR #115). **Session recovery context:** v1.4 (PR #103) clobbered the thin-shell `index.html` with a monolith; restored byte-identical via PR #104 (blob `501d79a`). Next refresh after QFs (Jul 9-11). |
