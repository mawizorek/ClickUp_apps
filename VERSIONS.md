...
| `inciardi-market` | shell v11 (verified) | live | Multi-page collector's terminal (terminal/catalog/collection/market + thin index router). v11 (PR #245, merge `977f606`): killed mobile horizontal overscroll (topbar nav `min-width:0` + internal `overflow-x:auto`, `overflow-x:clip` guard on html/body in `source/base.css`) and replaced literal `\u2192`/`\u2026`/`\u2014` + broken favicon-emoji escapes with real glyphs across all pages. Verified against fresh session read. |
