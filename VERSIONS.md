...
| `f1-racetracks` | v7 (verified) | live | Unified single-app shell. `index.html` = the REAL app: one persistent chrome (brand + Matrix/History/Circuits switcher) + ONE hash router (`source/app-shell.js`), with each lens mounted as an isolated, always-warm surface and cross-faded (NO reload, chrome never unmounts). `#matrix`/`#history` drive the standings surface; `#circuits`/`#circuits/<slug>` drive the circuits surface. Shell is same-origin so it ADOPTS each lens on load (injects chrome-hiding CSS + drives inner hash) — zero edits to the 24 lens modules; both still work standalone. Shell over literal merge because the lenses are classic-script bundles that share+depend on their own global scope and their `:root` tokens collide (`APP_VERSION` dupe, `--s1/2/3` opposite meanings). Prior: v6.3 hue-268 cohesion, v6.4 matrix density, v6.5 cross-lens View Transitions. PR #176. Verified via git blob API. |
