...
| `routines` | **v2** · 2026-07-27 · 🗑️ **DELETION CANDIDATE** | PR #560. Passive viewer of `routines/schedule.md` + the per-routine `last-run/*.txt` stamps. **INVOKE-ONLY** (scheduler retired 2026-07-26): the header states the MODEL and **derives** every count — **never put a time of day in it**, that is how v1 ended up advertising a wake schedule after nothing woke. `never` / unreadable / overdue stay visibly distinct **by rule**. 🛑 **Michael parked a delete-it question 2026-07-27** — it now answers the same question as Ricky's triage, off the same two files. **Not decided; app is live. But INVESTMENT IS FROZEN: do NOT do the `source/` split** (~20.8KB, over the 15KB line) **or add features** until the call is made. Steward: Fleet Felix. ⚠️ **This app and `schedule.md` are ONE SYSTEM** — reshaping that table is an app change. Case, counter-case, delete checklist + full v2 defect history: `routines/next-build-spec.md`. |
...
- **2026-07-27 (b) — `routines` flagged 🗑️ DELETION CANDIDATE, `source/` split FROZEN.** Michael, hours after v2 shipped: *"i don't need the fancy app as long as the schedule is findable and legible."* Worth a ledger-level note because of the shape of it: **the app and Ricky's invoked triage now answer the same question off the same two files** — two claimants on one truth, created not by anyone's mistake but by the scheduler's retirement removing the app's original reason to exist. **A capability change upstream can turn a healthy app into a duplicate overnight; nothing in this ledger would have flagged that.** Also trimmed the v2 defect narrative out of the row (it belongs in the spec — rows are current state).
- **2026-07-27 (a) — `routines` stamped v2** (PR #560), closing the **last `no version stamp` row** in the ledger. It had been rendering "Last run: never" for every routine since 2026-07-05 and quoting a wake schedule for a scheduler that no longer existed — **an unversioned app is an unverified app**, and both defects sat in the one surface whose entire job was reporting status. Also flagged on `world-cup-bracket` that its ROUTINE retired while the APP stayed live.
