...
| `f1-racetracks` | shell **v11** (`source/09`, `?v=20260801a`) · **23-round calendar 2026-08-01** · results store verified `d27ce55` (r1-9) | PRs #572/#573/#574 + **#654**. **🔴 THE DATA LAYER IS THREE VECTORS AS OF 2026-08-01 — read `f1-racetracks/README.md` ▸ Data Model before touching any JSON.** CIRCUITS (`circuits/<slug>.json`, timeless; the index is now a plain slug→file map) · **WEEKENDS (`season/2026/index_weekends.json`, the per-year join — THIS is the season calendar)** · CLASSIFICATIONS (`f1-results/2026/`, a manifest of rounds actually raced). **NOTHING TIME-SHAPED IS STORED:** round, status, display date and the current/last round pointers are all derived at boot by `source/08_season.js` from the session dates, so a cancellation or insertion self-heals. **🚨 NEVER PERSIST A DERIVED ORDINAL — join by `slug` or `cuTaskId`, never by round number.** Why: Bahrain + Saudi were cancelled mid-season 03-14, F1 renumbered densely, and the circuits index kept the old numbering with holes — **every entry from Miami on was off by two for eight months**, invisible only because a note told consumers to join by slug. **Sepang added** (hosts the *Bahrain* GP, 2-4 Oct; `circuits/sepang.json` is deliberately MINIMAL + sourced-only + `report:false` until its layout is dug). v7 screens LOCKED 07-26; **results-store** schema validated 07-28 (that stamp does NOT cover the circuit files — narrowed 08-01). ⚠️ Open: **`grid`+`qualifying` absent on r03/r04/r07** (needs SOURCED data — do not improvise), qualifying uneven in the other six, sprint arc documented-never-built, **`gp` migration to the weekend row half-done** (weekend wins, circuit file is the fallback), sprint `sess` labels disagree with the authoritative boolean on 5 circuits, no `sepang` in `LIVE_CIRCUIT_ALIASES`, **FOUR source modules over the 15KB line**, `applyTheme('f1')` unresolved, **three app version strings**. ⚠️ **RICKY: the refresh runbook moved** — `cuTaskId` lives on the weekend row and `current_round_slug` is gone; adding a round is now write the round file + one `{slug,file}` row. |
...
| `on-track` | shell **v2.4** (`?v=24`) · data 2026-07-23c | PRs #463/#468/#470. Data-separated. 2nd theme-spine consumer: composed 4-vector theme, **passive Theme picker** reading `shared/themes/_themes.json` live. **The reference implementation for a theme picker.** `:root` = labeled first-paint FALLBACK FLOOR. **The 20 series colors are a LOCAL data layer.** 🐞 **Open bug:** favicon/apple-touch/og:image point at `IMG_*.png` files that are GONE. One-line fix. |
...
- **`template-app` — gold standard, v5 (PR #300). Access: GATED, code `0426`.** The copy/place/audit baseline: slim hash-router `index.html` + `chrome.js` + `pages/` partials, styled off the `shared/themes` spine. Carries `CONFORMANCE.md` as the audit checklist.
...
⚠️ **Nor does coverage prove a row is TRUE, or CURRENT.** `f1-racetracks` was listed, versioned and hand-rewritten two days before its open-items list was found to describe work that had already shipped. **A maintained row is not a verified row** — stamp what you checked and when.

## Changelog

- **2026-08-01 — `f1-racetracks` v6.7.1 → v11** (PR #654). The season data layer split into three vectors and every time-shaped value made derived. **Durable findings:** *a store that can only contain things that ALREADY HAPPENED cannot be the calendar, so treating it as one guarantees a second, drifting calendar exists somewhere* · *a hand-maintained ordinal survives a normal season and dies on the first mid-season change — this one was wrong for eight months behind a "join by slug" note that read as a design property instead of a workaround* · **and the inverse of 07-28: the README held the CORRECT calendar while both data stores held the wrong one, so "open the JSON before you believe the plan" needs a return direction. Nothing in that app ever compared the two.** Also: the canonical Data Model named a table stored in `data.json`, deleted weeks earlier, while the file actually performing the join was absent from the table entirely.
- **2026-08-01 — `inciardi-collection` → v19** (PRs #637 + #638). Worker split (674 bytes under the read cap) + the artwork detail route. **Durable findings:** *a conditional answer whose condition is a build step needs that step written down AS a step* · *a split commit is where a quiet behavioural edit hides* · *a route class that needs a param belongs in neither of two lists that are both menus.*
- **2026-07-31 — `inciardi-collection` v3 → v17.** **Durable finding: validation proves a payload is WELL FORMED and can say nothing about whether it is TRUE — so a confirmation screen must render in the units of the physical thing, and rendering it is only half the fix, because a screen you cannot correct is a quiz.**
- **2026-07-30 — write key baked into the public bundle.** **Durable findings: a dashboard list is a UI, not the truth. And giving a setting a default silently kills the check that tested for its absence.**
...
_`unverified` rows carry the last value visible without a fresh read; each gets confirmed the next time that app is touched. Confirmed 2026-07-25: agentglass, prism, app-dashboard, markdown-viewer, world-cup-bracket, on-track, inciardi-market. Confirmed 2026-08-01: inciardi-collection (full module tree, both worker files, live `/health`); f1-racetracks (both season indexes, two circuit files, modules 09/12/14, `index.html`, `circuits.html`, live Pages read of the new weekend vector)._
