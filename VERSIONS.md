...
| `inbox-digest-report` | v2 (shell) · data: static seed 2026-07-16 | live | Data-audit surface for the Gmail Inbox Sweep. **v2 (PR #312): rebuilt from the gold-standard `template-app`** - real chrome (header, LEFT nav drawer, RIGHT settings drawer w/ live theme picker, footer stamp) + `styles.css` (template verbatim + `.matrix-*` grid block) + `pages/` partials. `index.html` = router shell (pointer, `DEFAULT_PAGE=matrix`, NAV = Field Matrix + About); `pages/matrix.html` = field-capture matrix (10 captured fields, live Jul 16 sweep rows) reskinned onto template classes; `pages/about.html`. maw-dark-utility default, swappable off the spine. Rolling `data/inbox-state.json` + dashboard page = follow-on (tracked in ClickUp). v1 (PR #311) was an improvised shell, superseded. |
