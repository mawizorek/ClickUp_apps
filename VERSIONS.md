...
| `routines` | 🗑️ **DELETED 2026-07-27 — NO LONGER AN APP** | PR #562. `routines/index.html` is gone; the folder survives as **docs only** (schedule, runbooks, `last-run/` stamps) and is listed here purely so the app is not re-added. **Do not rebuild it.** It rendered `routines/schedule.md`, which is now read directly or triaged by Routine Ricky — the app and his triage had converged on the same question off the same two files. Michael: *"i don't need the fancy app as long as the schedule is findable and legible"* + he never opened the URL. **Its lesson is generalizable and lives in `routines/next-build-spec.md` (tombstone): the app never rotted — retiring the scheduler is what turned it into a duplicate. When a capability is retired, ask what existed only to compensate for it.** |
...
⚠️ **Nor does coverage prove a row is TRUE.** `f1-racetracks` was listed, versioned, and rewritten by hand two days before its open-items list was found to be describing work that had already shipped. **A maintained row is not a verified row** — stamp what you checked and when.

## Changelog

- **2026-07-28 — `f1-racetracks` → v6.7.1 + open-items list struck** (PRs #572/#573/#574). A nine-lens schema pass read all nine round files and found this row, the app README, and the store's own manifest all describing work that had shipped five days earlier. **The fix that earned the version bump:** `positionsGained` was computed from a hardcoded 7-entry map, two of whose five checkable grid values contradicted the store. Added the open-items clause to the both-ways gate above and the maintained-is-not-verified clause to the coverage rule — **both are the generalizable half.**
- **2026-07-27 (c) — `routines` DELETED** (PR #562). Michael, on the question that decided it: *"no i don't open the routines url ever. and it's more important to keep single source of truth for the schedule."* App removed, launcher tile retired (`?v=73`), ledger row kept as a tombstone, pointers swept from `hooks/data-refresh.md` + Ricky's profile, and **`routines/schedule.md` rewritten for human reading now that nothing parses it.** Added the coverage-rule caveat above, which is the durable finding.
- **2026-07-27 (b) — `routines` flagged a deletion candidate, `source/` split frozen** (PR #561). Kept in the log because the *shape* recurs: the app and Ricky's invoked triage had come to answer the same question off the same two files — **two claimants on one truth, created by a capability change rather than by anyone's mistake.**
- **2026-07-27 (a) — `routines` stamped v2** (PR #560), closing the last `no version stamp` row. It had been rendering "Last run: never" for every routine since 2026-07-05 and quoting a wake schedule for a scheduler that no longer existed — **an unversioned app is an unverified app.** Also flagged on `world-cup-bracket` that its ROUTINE retired while the APP stayed live.
- **2026-07-26 (b) — TARGET RESET TO ~16KB, `Status` COLUMN DROPPED (Michael's ruling).** The ~12KB target had never been met since it was locked; per the Floor Rule a never-met target is the defect. New target = floor + real headroom, paid for by removing the duplicate `Status` column and trimming header narrative. Ceiling stays 22KB.
- **2026-07-26 (a) — `f1-racetracks` row rewritten for the v7 screen lock** (no version bump; docs-only, shell stays v6.7). **Indexed `inciardi-collection`.** Those two additions took the file 11.9KB → 13.3KB, triggering the reset above.
