...
| `inciardi-market` | shell v8 · catalog data 2026-07-10 | live | **Three-page app (PR #149): Catalog · Market · Collection, shared top-nav, same Pages link.** `catalog.html` = NEW visual print gallery (images, filters, search, on-eBay toggle, per-print detail drawer) reading `catalog.json`. `index.html` = market terminal, `app.js` UNCHANGED (still stamps v7/PR#96 — engine untouched, only the nav strip was added to the page). `catalog.json` expanded to a sourced image-backed set w/ corrected retail (big riso $70–$187). Worker/market pipe unchanged. Research SOP: `catalog-research-routine.md` (PRs #147/#148). `collection.html` still needs the shared nav strip added (next). |
