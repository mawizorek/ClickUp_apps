...
| `f1-racetracks` | standings lens v5 · circuit guide v5 · index.html router | live | **Canonical results store refactor (2026-07-09, this PR):** results now live ONCE in `f1-results/2026/` (per-round files + `index_rounds.json`, renamed from `index.json`). Both lenses read it: standings computes WDC/WCC live; the circuit guide derives podium/pole/FL live via `source/12_results_store.js`. **`data.json` DELETED** (orphaned + carried a wrong Albert Park FL + three stale podiums). `fastestLap` backfilled/corrected on every round from the ClickUp race tasks (Suzuka null = task not-confirmed; Silverstone absent = race task still a preview). `index.html` is now a slim router landing on the Championship Matrix, forwarding legacy `#/<circuit>` deep links to `circuits.html` (killed the duplicate circuit-app shell). ClickUp race task = canonical for NARRATIVE, repo store = canonical for RESULTS. **Lens integration not yet squared** (router is the seam); 2024/2025 historical backfill is a future ask that this per-season structure is built to absorb. |
| `world-cup-bracket` | shell v5.5 · data 2026-07-09 (verified) | live | Data-separated app: refreshes change only `data.json` (NO shell/version bump). **v5.5 (PR #106) — app-derived status:** the app now OWNS status. `normalizeMatches` in `util.js` derives upcoming/live/done at the load boundary (scores present → ft/`aet`/`pso`; kickoff passed & within window → live; else upcoming), so `data.json` carries FACTS only (scores, `psoNote`, `aet`, ET `time`, teams, `feedsTo`) and NO status string. Off-contract refresh values can no longer reach render. Data 2026-07-09: real QF kickoff times populated (ET), R16 complete (8/8). **Session recovery context:** v1.4 (PR #103) had clobbered the thin-shell `index.html` with a monolith; restored byte-identical via PR #104 (blob `501d79a`), then this v5.5 landed the derive adapter. Refresh contract (facts-only) to be mirrored into the World Cup Refresh ops guide. Next refresh after QFs (Jul 9-11). |
