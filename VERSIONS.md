...
| `inbox-digest-report` | v3.2 (shell) · data: Jul 17 2026 sweep | live | Data-audit surface for the Gmail Inbox Sweep. **Data refresh Jul 17 (PR #320): re-swept the live inbox — 8 threads (was 5).** New: Airbnb dormant-account nudge + Google security alert (own sign-in) → ALREADY_HANDLED; Marcus Vane via Risk Thresholds gold-pitch scam → SLOP; Joe re-sent the WisDOT email as the real source carrying the 3 files (Wisconsin DOT.pdf + 2 jpgs) → HAS_ATTACHMENTS (forward to task-18 email-in, then archive; prior ingested-flag draft no longer in inbox). Buckets: TO_MERGE 1 / NEEDS_REPLY 0 / HAS_ATTACHMENTS 1 / ALREADY_HANDLED 2 / SLOP 4; scoreboard 8→0. Slop clear adds `voice@cast.riskthresholds.com`. Still PLAN, pending greenlight. Only `pages/report.html` data changed; no shell bump. **v3.2 (PR #319): spreadsheet-dashboard skin for `pages/report.html`, mobile-primary.** Titled grid blocks (mono header bar + count connected to table), sharp corners, connected gridlines, dense mono labels. Fixed mobile wrap bug (`table-layout:fixed` + column widths + `overflow-wrap:anywhere`). Pills/chips/tags squared on `.report`; slop-clear a tap-to-select mono line. New CSS = `REPORT ADDITIONS` block appended to `styles.css`; `index.html` styles.css cache token → `?v=3`. Matrix page + matrix CSS UNTOUCHED. **v3.1 (PR #318): bugfix — slop clear query scoped to `in:inbox`** (a bare `from:(...)` searched ALL mail and surfaced archived + ACCOUNTS-labeled keepers + a receipt). **v3 (PR #316): added `pages/report.html` (Gmail Clean-into-ClickUp Pipeline Report) and made it the LANDING page.** Five locked buckets, inline gmail_url + task links, grouped slop + one-click clear, inbox-zero scoreboard, two-phase PLAN/EXECUTE spine w/ per-row status pill. `DEFAULT_PAGE` matrix→report, NAV = Report + Field Matrix + About. `data/inbox-state.json` fetch = next build. **v2 (PR #312): rebuilt from the gold-standard `template-app`** - real chrome + `styles.css` + `pages/` partials; `pages/matrix.html` = field-capture matrix; `pages/about.html`. v1 (PR #311) improvised shell, superseded. |
