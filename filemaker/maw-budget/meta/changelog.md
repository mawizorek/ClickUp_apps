...
1. **`README.md`** — cover page: purpose, goals, current status, the open questions.
2. **`meta/design-decisions.md`** — every committed call (DD-001…) with rationale + status. START HERE to understand WHY the app is shaped the way it is.
3. **`next-build-spec.md`** — the current plan: locked decisions, the object spine (shape only), phasing, and the remaining open inquiry.
...
**Current state (2026-07-15):** planning only, zero tables written on purpose. Next action = confirm DD-008 naming, then a dedicated field-articulation session writes the actual table/field definitions.
