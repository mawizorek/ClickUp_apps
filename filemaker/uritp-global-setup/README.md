...
The **config spine** of the URITP FileMaker package — the second shared backbone alongside the [People identity hub](../uritp-people/). Houses slow-changing organization reference data that every other app pulls via external data source: org identity (logo/name/acronym), fiscal years, academic periods, departments, and the Staff-Positions layer. Every app references this from day one; nobody hardcodes org name, logo, or fiscal year locally.
...
- Build the **Staff-Positions layer** here (owns Department + Title/level; seeds from as-built `_setup_Staff Positions` / `URJobProfileLevels` / `Supervisors`). Referenced by `uritp-people/ADULTS_ext.fkStaffPosition` (DP-006) and Labour.
...
- **Staff-Positions layer lives here** (DP-006, from the People session): org reference data both People (ADULTS_ext) and Labour read.

## Open Questions

- Staff-Positions exact shape: department + title + level + supervisor? (Confirm when building the layer.)
...
Shared configuration file referenced by all other URITP FMP databases via external data sources. The neutral spine: org identity, fiscal/academic time, departments, staff positions. Slow-changing reference data, not operational records.
...
- Define departments + staff positions as shared reference tables.
...
- Department + staff-position reference entries.
...
As-built reference: 6 tables built + in use (GLOBAL_USAGE_VARIABLES, PRODUCTIONS, Fiscal_Years, Academic Periods, Departments, + selector TOs). Target-state moves PRODUCTIONS OUT to the Productions builder and adds the Staff-Positions layer. This repo pass = target-state schema + relationships; layouts/scripts stubbed.

## Architecture Notes

One of the two shared spines (config here; identity in People). **Rule:** slow-changing org reference data lives here; operational entities with their own lifecycle (productions, roles, enrollments) live in their owning spoke. See `meta/design-decisions.md`.
