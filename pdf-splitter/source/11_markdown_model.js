...
(function () {
  var state = PS.state, cleanName = PS.cleanName, parsePages = PS.parsePages,
      pageToken = PS.pageToken, el = PS.el;
  ...
  function parseMarkdown(text) {
    ...
    blocks.forEach(function (block) {
      ...
      var entry = { name: "", start: "", end: "", folder: "" }; var hasAny = false;
      b.split(/\r?\n/).forEach(function (ln) {
        var idx = ln.indexOf(":"); if (idx === -1) return;
        var key = ln.slice(0, idx).trim().toLowerCase(); var val = ln.slice(idx + 1).trim();
        if (key === "page") {
          hasAny = true;
          var rng = val.match(/^(\d+)\s*-\s*(\d+)$/);
          if (rng) { entry.start = rng[1]; entry.end = rng[2]; }
          else if (/^\d+$/.test(val)) { entry.start = val; entry.end = val; }
  ...
  function rowVerdict(r, model) {
    ...
    var label = p.start === p.end ? ("Page " + p.start) : ("Pages " + p.start + "-" + p.end);
