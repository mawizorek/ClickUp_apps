...
3. **The single state file was 25,403 bytes and growing daily.** See the size-claim correction below — the *reason* recorded at the time was partly wrong, the *split* was still right.
...
## ⚠️ Size-claim correction (2026-08-04, same evening)

**What was written at migration time:** that the combined 25.4KB file had passed a **~22KB read-truncation line**, and that the runbook split was forced by a **~30KB return cap** — both quoted from `brain-config/README.md` → Verified Read Path.

**What was then observed, twice, in the same session:** `job-market-refresh.md` was read whole at **41,235 bytes** (v16, full changelog present) and again at **31,873 bytes** (v17.1, closing line present) through `githubmcp_get_file_contents`. **Neither truncated.** The 25.4KB TSV also read whole every single time.

**So: the cap is real as a documented risk but is NOT reproducible at these sizes with this tool, and it was cited as if it had already bitten. It had not.** Recording that plainly rather than quietly leaving a persuasive-but-unverified number in three files.

**Does the split still stand? Yes — on the reasons that survive:**

- **Per-role commits against a monolith are a payload problem, and that arithmetic is not in dispute.** Eight whole-file writes plus eight reads of one 25KB file is ~400KB of churn per pass, growing every day. Per-lane files make the boundary commit cheap. **This alone justifies the split.**
- **Concern separation.** A lane's rows are the natural unit of work; the loop already filtered to one lane at a time.
- **Truncation risk is real even if unmeasured**, and the failure mode is silent row destruction. Staying well under any plausible cap costs nothing.

**What does NOT stand:** any statement that a read *had* truncated, or that a specific byte count is a proven ceiling. If you need a hard limit, **measure it, do not quote it.** The lesson is the one already in Ricky's memory: *when you correct a claim, grep the claim, not the file* — this number is quoted in `job-market-refresh.md`, `README.md` and here, and this file is the one that owns it.

---

## Why per-lane files and not just per-role commits

Per-role commits alone were the ask. Costed out: tool count is cheap (+14 calls on a pass that already fires 40-60 fetches). **Payload is not** — see above. Splitting by lane means a role's commit touches only its own ~3-7KB file, at roughly today's payload.
...
`routines/job-market-state.tsv` (the old combined file) is a **tombstone stub**, not deleted — pointers elsewhere resolve to an explanation instead of a 404. Git history holds the original.
...
*Why:* duplicating a row into two files creates two claimants on one truth. The day it goes GONE you update one and miss the other, and the inventory quietly starts lying. Same failure that retired `registry.json` and got `data-refresh-log.json` deleted the day it was created.
...
🚫 **It is NOT for rejected finds.** Overhire, below-floor, box office, academic-teaching and out-of-industry all stay as prose in NOTABLE. Miss that distinction and this becomes a junk drawer inside a month — that is the whole risk of the file and the only thing that will kill it.
...
**A migration MOVES rows. It does not add them.** Three genuinely-qualifying finds surfaced in prose during the 08-04 pass were deliberately EXCLUDED so the count stayed verifiable. ✅ **All three were filed immediately afterwards, in a separate commit:**
...
- **Project Manager** — Rose Brand · Sun Valley, CA · $80,000-99,000 → `JM-OSJ-rosebrand-pm` in `_unfiled` (the first row the holding pen ever took)
...
| 4 | `job-market-refresh.md` | → v17 (per-role commit, split state, `also_lanes`, `_unfiled`, CHECKPOINT rewritten), then **v17.1** (the Resume Test) |
...
| 8 | `job-market-templates.md` | Template 8 rewritten (position-only), Template 9 (SPENT) added |
| 9 | task `86ajtgbt3` description | rewritten as the v17.1 trigger contract, Resume Test as step zero |
