...
| **F1** · `f1-refresh.md` | **Thu–Sun**, session-aware | `last-run/f1.txt` | Eligible on any invocation Thu–Sun, but the runbook decides: it only refreshes if an F1 session has actually finished since the last stamp. Otherwise a clean no-op. **Summer break through Aug 21 (Dutch GP Zandvoort, sprint weekend).** During break: eligible once/week (any day) as a pulse; still session-gated so always a clean no-op until sessions resume. ⚠️ **2026-08-22: the session-aware check fires on ANY finished session while every downstream step needs a completed ROUND — a finished practice satisfies the trigger and gives the routine nothing to write. Surfaced by Zandvoort’s sprint weekend. 🔻 Proposed fix lives with the runbook, not here.** |
...
| **Agent Memory Report** · `agent-memory-report.md` | **Weekly** (any day) | `last-run/agent-memory-report.txt` | Fleet-wide memory health diagnostic. READ-ONLY: measures all agent bundles against rotation budgets, generates a prioritized report for Memory Maggie. Watchlist: `routines/agent-memory-watchlist.md`. Measurement spec: `brain-config/super-agents/_shared/bundle-measurement-spec.md`. |
...
- **Weekly cadence** (Agent Memory Report): due if last-run is older than 7 days. Same catch-up logic: one pass covers any gap.
...
- **`job-market-refresh.md` — AFFECTED, and it already half-agreed.** Its Resume Scan step 7 and matching guardrail already declare a pass ABANDONED when the newest comment is older than one cadence interval (>24h for a daily routine). **Same intent, wrong unit.** 🔻 **OWED: replace step 7’s rolling test with the calendar-day test.** Until then, **THIS section is the rule and it supersedes the rolling-interval wording there.**
  - 🩹 <s>⚠️ **That file could not be corrected in the same pass — it is 34.6KB, over the hard 30KB `create_or_update_file` cap (`GitHub MCP — Operating Standard`), so an agent cannot safely rewrite it.** 🔻 OWED: … and split the runbook back under the write cap.</s>
  - 🔴 **THE CAP WAS PHANTOM. STRUCK 2026-08-22 (Dev Dexter) after measuring, not after reasoning.** `job-market-refresh.md` at **34,668 B read back WHOLE** via the git blob API on 2026-08-22 — complete frontmatter through the v12 changelog entry, nothing clipped. On the same day `on-track/data.json` was **WRITTEN at 34,783 B** through `create_or_update_file` and read back whole and valid. **Two independent measurements, one read and one write, both past the “hard cap,” both clean.**
  - ⚠️ **This claim blocked real work for 16 days and it was never tested once.** It was written 2026-08-06 and cited as the reason Job Market could not run on 08-09, 08-17 and 08-22 — three triages named it, and the routine went from 4 days stale to 15.4. **A blocker nobody re-tested is indistinguishable from a blocker that does not exist.**
  - 🔴 **It is also the exact defect `README.md` rule 14 already forbids, in the file next door:** *“Corrected 2026-08-04: that number was quoted, not measured, and was NOT reproducible — a 41KB and a 31.8KB file both read back whole the same day… **Do not re-introduce a byte threshold you have not measured.**”* This section re-introduced it **two days later**. The rule and its violation have been sitting one file apart ever since. ⭐ **The generalisable part: a promoted rule does not protect the file that promoted it.** README rule 14 was promoted OUT of this very framework and then contradicted inside it, because nobody re-read the parent when writing the child.
  - ✅ **The real rule is the behaviour, never a constant** (`README.md` rule 14): **if a file did not come back whole, STOP and never write from a partial read.** That test costs one extra read and it works at every size. A number does not.
  - 📐 **Splitting is still defensible on its own merits and is now a CHOICE, not a forced move.** A whole-file rewrite of a monolith at every boundary is a payload problem that grows daily, and 34.6KB is genuinely awkward to edit. **But it is no longer a prerequisite for running the routine, and it must never again be quoted as one.** Precedent worth copying: `brain-config/session-board.md` went 30,990 B → 6,374 B by moving durable content to a `session-board.notes.md` sidecar, and the ceiling is now enforced by `.github/workflows/size-budget.yml` instead of by prose.
...
- ⚠️ **A BLOCKER IS A CLAIM, AND CLAIMS EXPIRE (added 2026-08-22).** Before citing a previously-recorded blocker as the reason a routine did not run, **re-test it**. If it cannot be re-tested cheaply, say that instead of restating it as fact. The 30KB write cap above was quoted across three triages and sixteen days without one measurement, and it was false the whole time. **Carrying a blocker forward is not the same act as verifying one.**
