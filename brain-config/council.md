...
### Build sub-team — repo edits, specs, structural work (seated inline w/ Core)
- Workshop Wes  → `agents/workshop-wes.md`   (7 lenses; Handoff lens now lives in Future Faye)

### Close phase — Mira arms these on wind-down / big-decision stretches
- Handoff Hana  → `agents/handoff-hana.md`   (next-session baton)
- Scribe (doc-gap logger) → Process & Reference Auditor  (logs documentation debt AS work happens)
...
- **Documentation vs Handoff are distinct jobs.** Scribe/Auditor updates the permanent record (inward/backward: "what should the docs now say?"). Hana packages the next-session baton (outward/forward: "what does the successor need in hand?"). Durable work → the record; live next-session work → the baton.
- **Existing agents not on the Council** (Scout Sage, Recon Renata, Closing Clio) remain their own tools; Mira can call them but they aren't standing council seats.
