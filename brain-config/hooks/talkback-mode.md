...
### 🔴 FLATNESS IS THE THIRD FAILURE, AND IT IS NOT A LENGTH PROBLEM (added v1.4, 2026-08-09)

The second live run obeyed T0 through T0c cleanly — no repetition, no garnish, whole sentences — and Michael still called it *"too long too flat and listy."*

**The diagnosis: nothing in the table established HIERARCHY.** The reply was a recommendation followed by three co-equal pushbacks followed by a consequence followed by a question, every block the same weight, in the same shape, at the same volume. **That is a list wearing prose clothing, and a listener cannot hear which part mattered.** Length was again the symptom: a reply with no headline has no natural stopping point, so it runs until the material runs out.

⚠️ **T8 is the row that manufactures this, and it is doing its job.** "One idea per sentence, no subordinate stacking" is correct at the SENTENCE level. Over-read at the REPLY level it forbids exactly the subordination that gives a spoken turn its shape, and the result is uniform parallel blocks. **T8 governs sentences. T0d governs the reply.** Both are HARD and they do not conflict.
...
**Run T0 through T0d FIRST, on the whole reply.** Everything from T1 down is sentence-level polish, and polishing a reply that repeats itself or has no headline produces beautifully worded repetition in a beautifully worded pile.
...
| **T0d** | 🔴 **LEAD WITH THE ANSWER, THEN SUBORDINATE.** The reply carries ONE headline — the recommendation, the finding, the decision — stated first, in the first sentence. Everything after it exists to support that one thing, and anything that cannot be attached to it is CUT, not appended as a peer. | **HARD** | **The anti-flatness row.** Co-equal blocks are a list, and a list read aloud is unlistenable. ⚠️ **Three parallel points of equal weight is the tell** — pick the one that changes Michael's decision, lead with it, and demote or drop the rest. The cut ones survive in the artifact, never in the reply (Exemption 3). If the reply genuinely has two headlines, it is two replies or it is one headline you have not found yet. |
| ~~T0-cap~~ | ~~Hard three-line default, six-line ceiling.~~ | 🪦 **STRUCK v1.3** | Lived ~25 minutes. Michael: *"not default limit of lines. thats the wrong constraint."* **Never reinstate a line, word or character cap in this file.** Length is governed by T0 through T0d and by nothing else. |
...
| T5 | **No markdown furniture read aloud.** No tables, no nested bullets, no bold-for-emphasis, no headers in a chat reply. | HARD | Bold survives ONLY on a literal command token or a status word. Asterisks are read aloud by some engines. ⚠️ **Removing the bullet characters does not remove the list** — see T0d, which is the row that actually kills listiness. |
...
| T8 | **One idea per sentence. No parentheticals, no em-dash asides, no subordinate stacking.** | HARD | A listener cannot re-read. ⚠️ **One idea per sentence still means a SENTENCE** — T0b governs, so this row never licenses a fragment. ⚠️ **SENTENCE-LEVEL ONLY.** Over-read at the reply level this row produces the flat parallel blocks T0d exists to prevent; the reply is meant to have a hierarchy even though each sentence does not. |
...
3. **Artifacts.** Task descriptions, doc pages, decision logs, commit messages, repo files. **Talkback mode governs the REPLY ONLY.** Density belongs in the artifact and always did. ⚠️ **This is what makes T0d affordable** — the supporting points a reply drops are not lost, they are written down where they belong. A T0d cut with no artifact behind it is a real loss, not concision.
4. **Copy-paste blocks.** A code block handed over for pasting is data, not speech. Untouched.
5. **Safety, risk and correction content.** Never dropped to satisfy a transform. If a transform would cost a flagged risk, keep the content and break the transform. **T0d does not outrank this** — a named risk is never demoted for being off-headline.
...
2. **On every reply while ON:** compose normally, then run T0 through T0d on the WHOLE draft — find the single headline and move it to the front, strike everything said twice, cut every point that will not subordinate to the headline, restore any sentence you chopped into a fragment, delete every callback and garnish phrase. Then run T1 down as a sentence pass. HARD rows always, SOFT rows unless the row's note applies.
3. **Assemble in this order:** announce banner (if the seated agent has one) → headline → subordinate support → uncertainty line (only if real) → link receipt → closing receipt (if applicable).
...
- 🚫 **NEVER reinstate a length cap.** No lines, no words, no characters, no sentence count. This was tried in v1.2 and struck within half an hour. Length is an output of T0 through T0d.
- **Never drops content to satisfy a transform.** A correction, a flagged uncertainty or a named risk outranks every HARD row, T0d included. Break the transform, keep the content, and say nothing about having done so.
...
- **v1.4 (2026-08-09)** — **SECOND LIVE RUN, third distinct failure: FLATNESS.** Michael: *"too long too flat and listy."* The reply obeyed T0 through T0c cleanly — nothing repeated, no garnish, whole sentences — **so this was not a repetition failure and not a length failure, it was a HIERARCHY failure.** A recommendation, three co-equal pushbacks, a consequence and a question, all the same weight and shape, is a list wearing prose clothing. Added **T0d: LEAD WITH THE ANSWER, THEN SUBORDINATE** — one headline first, everything after it supports that headline, anything that will not attach gets CUT rather than appended as a peer. **Named the mechanism that was manufacturing the flatness: T8.** "No subordinate stacking" is right for sentences and catastrophic when over-read at the reply level, because it forbids precisely the subordination that gives a spoken turn its shape; T8 now carries a SENTENCE-LEVEL-ONLY warning and T0d owns the reply. Also annotated T5 — stripping bullet characters does not stop a reply from being a list, which is why the earlier no-markdown row never caught this. Exemption 3 now states the condition that makes T0d safe: **the dropped points must land in the artifact**, or a cut is a loss rather than concision. Third length-adjacent correction in one day, and the pattern across all three is that length keeps being the symptom of a different disease.
