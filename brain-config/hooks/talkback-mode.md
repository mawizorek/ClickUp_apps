...
**Purpose:** A session-scoped dial that renders every reply as an explicitly worded VERBAL UPDATE meant to be heard, not read: short, conversational prose stripped, emoji stripped, maximum directness. This file is the single tuning surface for how strict that rendering is.
...
🔴 <s>**What talkback mode is NOT: shorter.** Spoken Voice already handles length. This changes the WORDING, not the word count.</s> **STRUCK ON FIRST CONTACT (2026-08-09, Michael: *"that it was like times too long to begin with"*).** The v1 demo obeyed all thirteen transforms and was still several times too long, which means the premise was wrong, not the execution. **Length IS the register.** A spoken update that runs long is a bad spoken update no matter how clean each individual sentence is — a listener has no skim. Length is now T0, the first and highest transform, and it OUTRANKS every wording rule beneath it. The surviving half of the old premise: talkback mode still never buys brevity by dropping content. It buys it by dropping SCOPE — fewer things said, not the same things said in fragments.
...
**Run T0 FIRST and run it on the whole reply.** Everything from T1 down is sentence-level polish, and polishing a reply that is four times too long produces a beautifully worded reply that is four times too long.

| # | Transform | Strength | Notes |
| --- | --- | --- | --- |
| **T0** | 🔴 **LENGTH CEILING. Default reply is THREE spoken lines. Hard ceiling six.** Over six lines, the reply is wrong — cut scope, not words. | **HARD, outranks all** | The single most important row and the one v1 was missing. Count LINES a person would speak, not sentences on screen. A status update is one line. A decision plus its reason is two. Three is a full answer. **If it will not fit, the extra belongs in the artifact and the reply points at it.** |
| T0b | **Say the ONE thing. Not the one thing plus its supporting cast.** | HARD | The v1 demo listed what left, what stayed, and what to notice. All true, none asked for. **Cutting scope is how T0 is met; cutting words is how T0 is faked.** |
...
| T7 | **Links move to a closing receipt.** Body text carries zero inline links; the reply ends with a short labelled link list. | HARD | Preserves the memory rule that links stay prominent, without a URL in the middle of a sentence. ⚠️ The receipt counts against T0. Two links, not five. |
...
| T11 | **Uncertainty is stated as a labelled line, never as a softened sentence.** "Unverified:" or "Open question:" | HARD | The one place where talkback mode ADDS words. Directness must not read as false confidence. ⚠️ It costs a line against T0, so it fires on REAL uncertainty only, never as a habit. |
...
3. **Artifacts.** Task descriptions, doc pages, decision logs, commit messages, repo files. **Talkback mode governs the REPLY ONLY.** Density belongs in the artifact and always did. ⭐ T0 makes this load-bearing rather than advisory: a three-line ceiling only works because the detail has somewhere to go.
4. **Copy-paste blocks.** A code block handed over for pasting is data, not speech. Untouched.
5. **Safety, risk and correction content.** Never dropped to satisfy a transform. If a transform would cost a flagged risk, keep the content and break the transform. **T0 included — a risk that needs a fourth line gets a fourth line.**

---

## Procedure

1. **On invocation:** set the session flag and confirm in ONE line, in talkback mode. **Do not demonstrate the mode by describing it.** The v1 procedure said to "speak the confirmation so the register is demonstrated," which produced a fourteen-line essay about brevity. Confirm and stop; the next real reply is the demonstration.
2. **On every reply while ON:** compose normally, then run T0 on the WHOLE draft first — cut scope until it fits. Then run T1 down as a sentence pass. HARD rows always. SOFT rows unless the row's note applies.
...
- **Never drops content to satisfy a transform.** A correction, a flagged uncertainty or a named risk outranks every HARD row including T0. Break the transform, keep the content, and say nothing about having done so.
...
- **v1.2 (2026-08-09)** — **FIRST LIVE RUN, and it failed on the one axis the file explicitly declared out of scope.** Michael, on the demo reply: *"that it was like times too long to begin with."* The demo obeyed all thirteen transforms and was still several times too long, **so the failure was the PREMISE, not the execution.** v1 said in bold *"What talkback mode is NOT: shorter — Spoken Voice already handles length"*; that sentence is struck. **Delegating length to the floor was the error:** the floor says "talk in beats," which is a direction, not a ceiling, and a direction cannot be violated. Added **T0, a hard three-line default with a six-line ceiling, ranked above every wording rule**, plus **T0b: cut SCOPE, not words** — the demo listed what left, what stayed, and what to notice, all true and none asked for. Also killed the v1 procedure step that told the agent to demonstrate the register in its confirmation, which is what produced a fourteen-line essay about being brief, and added the guardrail that follows from it: **never explain the mode while in the mode.** T7 and T11 annotated to note they spend lines against T0. Ratio of the finding to the fix is the lesson worth keeping: **one live invocation taught more than the entire build.**
- **v1.1 (2026-08-09)** — **RENAMED `verbal-mode` → `talkback-mode` by Michael, ~20 minutes after v1 merged and before the hook had ever fired.** File moved, `verbal-mode.md` deleted outright rather than tombstoned: a stub for a file with a twenty-minute life and no inbound pointers is noise, and every reference to it was rewritten in the same pass. Dead tokens `/verbal-mode` and `/verbal` recorded at the top as resolving here, because PR #777 preserves them in history and a cold agent following that trail needs a landing place. **Added the NAME DISAMBIGUATION block:** "talkback" already means the post-show audience Q&A (a live URITP event type) and the booth-to-stage intercom, so the file states up front that the hook is neither and only fires as a slash command. The intercom sense is the intended metaphor and is the reason the name is better than the one it replaced — terse, direct comms over headset is exactly the register. PR #780.
