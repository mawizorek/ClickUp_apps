...
> - **Universal refresh rules** → `routines/README.md` (Data-Refresh Discipline)
>
> This file contains **none** of those and must never grow them back. It holds exactly one thing: **what an INVOCATION does** — read that framework, do the arithmetic, and propose. Everything else is a pointer.
...
## 📌 Stamping (MANDATORY after any run — by ANYONE)
...
- **A FAILED run leaves the stamp untouched** so it stays overdue and self-heals — and is reported in the reply, not by DM.
...
The full floor is **Data-Refresh Discipline** in `routines/README.md` (12 rules) — read it there, it is not restated here. `hooks/source-freshness-gate.md` is folded in as rule 10 and is **fire-always** the moment a routine fetches.
...
- **READ-ONLY except the stamp.** Landing a result somewhere new is a separate explicit instruction.
