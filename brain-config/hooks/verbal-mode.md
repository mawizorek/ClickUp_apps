...
**Invocation:** `/verbal-mode` · `/verbal` (aliases, both = ON) · `/verbal-mode=off` (canonical off) · `/conversational` (alias for off). Mid-session invocable, toggle freely, persists for the session.
...
**What verbal mode is NOT: shorter.** Spoken Voice already handles length. This changes the WORDING, not the word count. A stripped reply that drops a named risk has failed, exactly as it would on the floor.

---

## 🎚️ The Transform Table (THE tuning surface — tune HERE, nowhere else)

Each row is one transform. Each has a strength: **HARD** (always applies in verbal mode) or **SOFT** (applies unless it would cost content). Turning the dial = editing a strength, adding a row, or striking one. Strike rows, never delete them, and note who ruled it.
...
| T6 | **Speak paths, IDs and URLs, do not spell them.** "the verbal mode hook in brain config" not `brain-config/hooks/verbal-mode.md`. | HARD | The worst TTS failure mode in this workspace. A slug read character by character is unusable audio. |
...
| T11 | **Uncertainty is stated as a labelled line, never as a softened sentence.** "Unverified:" or "Open question:" | HARD | The one place where verbal mode ADDS words. Directness must not read as false confidence. |
...
| T13 | **Direct address stays.** Using Michael's name, and second person, are kept. | HARD | Verbal mode is not impersonal. It is a person speaking plainly, not a system printing a log. |

---

## ✅ Exemptions (things verbal mode does NOT touch)
...
2. **Quoted speech, proper nouns and domain terminology.** Never rewritten. A quote in verbal mode is still a quote.
3. **Artifacts.** Task descriptions, doc pages, decision logs, commit messages, repo files. **Verbal mode governs the REPLY ONLY.** Density belongs in the artifact and always did.
...
1. **On invocation:** set the session flag, confirm in ONE line, and speak that confirmation in verbal mode so the register is demonstrated rather than described.
...
4. **On `/verbal-mode=off`:** clear the flag, confirm in one line, revert to the Spoken Voice floor.
...
- **Explicit invocation only.** Never infer verbal mode from context, from a task type, or from the fact that a previous session used it.
...
- **Never touches an artifact.** Reply surface only. An agent that strips a decision log because verbal mode was on has inverted the entire Documentation Instinct.
