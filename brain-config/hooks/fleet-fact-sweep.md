...
**Guardrails:** read-only until a claim is confirmed against the ladder · never verify a fleet fact from a neighbouring file · never read a retired manifest as data · fix facts freely with provenance, flag every lane and stance call · never rewrite another agent's voice or conclusions · report VERIFIED CURRENT as well as failures · **no Register rows in this file** — data lives in the Register, procedure lives here.

**Changelog:**

- **v1.2 (2026-08-01)** — Register split out to `super-agents/fleet-known-drift-register.md`. Two reasons: this file had crossed **22.3KB, past the unreadable-whole ceiling it warns about**, and the v1.1 ownership ruling had just given the Register a different steward than the tool. Procedure vs data, the same split `routines/` uses. ⚠️ **Third size correction on one file in one session** — 21.4 → 19.4 → 18.3 → 22.3 → ~11KB. Every miss came from estimating instead of reading the returned byte count.
