...
**Status:** LIVE runbook (not a proposal). Authored 2026-07-17 from the Audit Anna reference build.
...
**Reference implementation:** `brain-config/agents/audit-anna/` (the first migrated agent — copy its shape).

> **Purpose of THIS doc:** let a Brain session with ZERO prior context pick up ONE agent and migrate it flat→folder quickly, correctly, and identically to every other agent — with no coordination call to Michael. Read this top-to-bottom, do the steps, done. If a step surprises you, the runbook is wrong — flag it to the steward, don't improvise.
...
The upgrade spec assumed a **big-bang single-merge** (all ~27 agents flip in one atomic PR, main never mixed). Michael's 2026-07-17 direction supersedes that: migrate **incrementally, one agent per session, over the next few weeks.** That reversal has one hard consequence you MUST honor:
...
## 2. Prerequisite (ONE-TIME, before ANY further agent migrates)

Before agent #2 moves, dual-resolution must be live in the viewer read paths and the pointer conventions. This is the single build task gating the whole incremental run. Do it once, on its own PR:
...
Until this PR merges, the ONLY migrated agent is the reference (Anna), and her flat twin stays live as the fallback. **Do not migrate a second agent before the prerequisite lands.**

---

## 3. The per-agent recipe (the cold-open steps)

For a single `<slug>`, one branch, one PR. This is exactly what the Anna reference build did.

### Pre-flight
1. Load `GitHub MCP — Operating Standard` (blob-API reads, branch→PR→self-merge, `.nojekyll`, VERSIONS ledger). Non-negotiable.
2. Confirm the prerequisite (§2) is live. If not, STOP — you're the wrong task.
...
6. **`<slug>/profile.md`** = the flat body with EXACTLY two changes: (a) remove the `## Changelog` section, (b) append the footer line `*Version history: see `change-log.md` (sibling). Git holds the detailed diffs.*`. Nothing else moves. Front-matter, every rule, every section, verbatim. If you're tempted to "improve" anything — STOP, that's a separate audit pass, not this job.
7. **`<slug>/change-log.md`** = the extracted `## Changelog` content under a `# <Display Name> - Change Log` header + the standard one-liner (copy the shape from `audit-anna/change-log.md`).
8. **`<slug>/metadata.json`** = the sidecar content, verbatim.
...
10. `registry.json` — the `profile` field → `<slug>/profile.md`. **REGENERATE via its generator; never hand-edit.** (It's the sanctioned mirror-pair with the AI Toolkit index — reconcile both sides same-session.)
...
- `registry.json` regenerated (not hand-edited); `council.md`, index trigger table, cross-agent refs all point at the folder.
...
- **Storage-only.** No agent's behavior/rules change during migration. A diff of the actual rules pre/post must be empty. Do NOT condense, reword, or "fix" a profile mid-migration.
...
- **Never half-migrate an agent.** Folder + flat-deletion land together or not at all.
- **One agent per PR** during the incremental phase — clean revert, clean diff, cold-pickup friendly.
...
**Audit Anna owns this runbook.** When an agent's migration surfaces a new wrinkle (a nonstandard sidecar, a profile that won't split cleanly, a pointer nobody inventoried), Anna updates §6 and the affected step — that's her template-authoring role, not a Michael decision. The runbook is the single source of truth for the transition; if it and reality disagree, reality wins and Anna reconciles the doc.

---

*Authored 2026-07-17 from the Audit Anna reference build (`brain-config/agents/audit-anna/`). Companion to `agent-folder-upgrade.md`. Steward: Audit Anna.*
