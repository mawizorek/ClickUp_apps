...
**Status: PROPOSED 2026-08-01 (authored by FMP Fiona at Michael's direction, having run it on herself first). Pending Fleet Steward (ClickUp Coach Corey) ratification before it governs fleet-wide rollout.** `fmp-frank` is the reference conversion; this runbook is the generalized procedure derived from doing it.
...
3. **Land the kernel + this runbook** in the repo (PROPOSED status until the Steward ratifies rollout).
...
- [ ] kernel + runbook landed (PROPOSED)
...
**Fleet order so far:** `fmp-frank` (reference, done 2026-08-01) → Corey (next, and he ratifies this runbook on his own pass) → Milo, Listing Lookout.