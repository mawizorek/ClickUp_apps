...
⚠️ **2026-08-11 — A LIVE POINTER SURVIVED BOTH REMEDIATION BATCHES, IN THE STEWARD'S OWN BUNDLE.** `super-agents/mainstage-milo/README.md` still reads *"Canonical metadata lives in `../superagents.json`. Do not mirror fields here"* as a LIVE instruction with no strikethrough — pointing at a stub retired 07-30. ⭐ **A tombstone read as canon returns EMPTY, and an empty read satisfies "do not mirror fields here" perfectly. The instruction succeeds and teaches nothing.** ⚠️ **Fix owed; not fixed in the 08-11 passes because both were scoped to activity logs.**
...
**Current: peer by default**, and Michael rules explicitly when it matters. Standing rulings: **Milo ↔ Tate are PEERS** with a seam that has NO owner (07-30) · **Riley READS Fiona's schema, comments on it, and never RULES on it** (08-01) · **the eleven Production Office heads are a different AXIS from Milo, not a tier under him** (08-01, Naming DL S2). Subordination language is a finding unless a dated ruling backs it.
...
🔴 **AND THE INVERSE TRAP, EARNED THE SAME DAY IN A CLICKUP LIST: AN EMPTY CONTAINER IS NOT AN ABSENCE.** `URITP Inventories ▸ PROP Inventory` holds **exactly one row**, which reads as *the program owns no props* and actually means *nobody itemized props.* ⭐ **Same shape as the URITP Paperwork family, which is fully scaffolded and almost entirely empty.** **Never quote an empty list as evidence that the thing does not exist — check whether the list was ever populated.**
...
🔴 **AND THE SAME MORNING PROVED THE OTHER HALF TWICE: A WRITE CAN PUT A FILE PAST ITS OWN READ CEILING.** Milo's `activity-log.md`, one file, three measured writes in ninety minutes: **33,565 B** (over the ~30KB `create_or_update_file` cap the operating standard LOCKS — the write SUCCEEDED, which is luck rather than permission) → **25,638 B** after rotating four entries out → **29,986 B**, which is **fourteen bytes under the hard cap**, on the very next write → **26,644 B** only after content was moved out of the repo entirely. ⭐ **Two near misses on one file in one morning means the file is not the problem. A sliding window CANNOT control a file whose growth is happening in the part that does not slide** — his LIVE STATE block is a permanent fixture outside the window by design and is now the largest thing in the file. **Rotation is not a fix for it. That needs a ruling.**
...
🔴 **THIS ROW FIRED AGAIN FOUR HOURS AFTER IT WAS WRITTEN, IN THE OPPOSITE DIRECTION, AND THE SECOND INSTANCE IS THE MORE DANGEROUS ONE.** The 08-11 write-back PR (#806) recorded four items as **still owed** — three demand rows *"that do not exist yet"* plus a missing department tag. **Every one of them was already done.** The three rows had been created **01:45–01:46 ET, three to four minutes after the meeting that named them closed**, and they were not stubs — each carried a full body. The tag had been applied too. ⭐ **So the row is bidirectional: a DISCHARGED gate can read as owed, and a COMPLETED artifact can read as missing. Both directions produce phantom work.**

⚠️ **The second direction is worse because a false OPEN item invites duplicate creation.** The next session to read *"the deck-finish row does not exist"* creates a second deck-finish row, and now the ledger has two. **The 08-11 pass avoided that only because it read the list before writing.** ⭐ **Before acting on ANY inherited `Owed:` line, open the container it refers to. Not the meeting it came from — the container.**

🔴 **Root cause of the FIRST direction is a MISSING STEP, not a habit: nothing in the seat → open → synthesize arc says "and now each voice records what it learned."** ⭐ **A POST IS NOT A WRITE.** ⚠️ **A write-back clause in `_shared/department-head-base.md` plus a chair-side duty in `council.md` is PROPOSED and unruled; a shared-base change is Michael's call.**

🔴 **Root cause of the SECOND direction is different and needs its own guard: the operator closes gaps in MINUTES and does not announce it.** ⭐ **Anything recorded as *owed by Michael* has a shelf life measured in minutes, not days. Re-read before reporting, never report an owed state from a transcript.**
...
### D18 · 🆕 Whether a mechanism you are about to propose already exists
**Current (earned 2026-08-11): check the STATUS SET and the FIELD LIST before designing a surface — twice in one morning a mechanism was nearly rebuilt that was already live and in use.**

1. 🔴 **`waived` — described in a durable memory file as *"the status most of our surfaces lack"* — is a live status on `Script Demands (BL)` and a row is sitting at it.** ⚠️ **AND THE SCHEMA READ DID NOT REPORT IT:** `DESCRIBE` returned five statuses totalling 37 tasks against **38 live rows**, silently omitting the one a row was actively using. ⭐ **A schema census is a projection with a blind spot, exactly like an aggregate view. If the status counts do not sum to the row count, a status is missing from the census, not from the list.**
2. 🔴 **A `wish list` status already exists in `URITP Inventories` and the scene shop is already using it** — found while preparing to propose a wants-vs-has mechanism for a department head's shop wish list. **The mechanism was built.**

⭐ **The general form, and it is the twin of D14: before you design, ask whether the workspace already encodes this — in a status, a field, a task type, or a punctuation convention.** ⚠️ **This is the same failure recorded twice before in a different register: *"there is no process for X" does not license "there is no X"* and *a schema says what CAN be stored, only the rows say what IS.*** **Three instances, three months, three domains. Treat it as a standing pre-build check.**
...
- **2026-08-11 (second pass, ~4h after the first)** — 🔴 **D17 gained its OPPOSITE DIRECTION and is now bidirectional**: the same PR that recorded four discharged gates as owed also recorded four COMPLETED artifacts as missing, and the completed ones had been finished within four minutes of the meeting that named them. **D18 added** (a mechanism that already exists — `waived` and `wish list`, plus the status census that dropped a status a row was using). **D15 gained the write-past-the-read-ceiling case** measured across three writes to one file in ninety minutes, one of which landed fourteen bytes inside a hard cap. **D11 gained the inverse trap** (an empty ClickUp list is not an absence — `PROP Inventory` holds one row). **D8 gained the axis-not-tier ruling.** Delta driven by executing the seating's owed items and discovering they were not owed.
- **2026-08-11** — **D16 added** (a build-time LIVE STATE stamp is the least trustworthy kind; eleven simultaneous instances, two rotted within hours of being written). **D17 added** (an `Owed:` item already discharged; four heads at once, plus the partial-discharge discipline Hawthorne demonstrated). **D15 gained two confirmations.** **D2 gained a live-pointer finding** (`mainstage-milo/README.md` still points at the retired `superagents.json` stub with no strikethrough, surviving both 08-01 remediation batches). **D3 gained a narrow legitimate exception** (a ballot-closed set of eleven is not a fleet count). **D5 gained a ten-day-old "pending"** on Milo's native shell. **D11 gained a correctly-empty ledger** (Hawthorne's incident memory) **and a wrongly-empty one** (Gable's Ledger D). Delta driven by the Big Love eleven-head seating backfill.
