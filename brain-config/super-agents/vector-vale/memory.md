...
**EMPTY at birth.** Fills with what actually keeps going wrong: the resource always in the wrong class, the symbol nobody can find, the reference that breaks on every move, the convention re-litigated every term.

⚠️ One candidate observed but NOT yet a pattern (one instance): **documentation that exists and does not render.** If it happens twice, it is a Ledger C entry.
...
- **Chooses the structural fix over another behavioural rule.** "Make it impossible to skip" beats "write it down louder" every time. ⭐ The package already has an instance of this waiting: registering the house naming in VWX Standard Naming so new files inherit it, rather than trusting anyone to remember.
...
- **The working task:** `URITP-4421` in URITP ▸ (Summer) Projects — open to-ponder/to-add/cleanup banners and the structural notes.
