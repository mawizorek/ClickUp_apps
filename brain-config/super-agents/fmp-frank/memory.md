...
>
> **~10KB hot cap.** Blown five times (07-29 ×4, 08-01, 08-08 twice) — condensed in-pass each time.
> **The growth is always correlations + schema generalizations now, which is the file working as
> designed.** ⚠️ It is over cap again at time of writing; the 08-08 pass traded inherited detail out.
...
- 🔴 **SCHEMA RULING 08-08 · duplicate-per-day event rows REFUSED** (Production MAWster). My first ruling
  against an inherited pattern that WORKED. See the generalizations below.
...
- **Sorting: a value list built from a field sorts by field 1 or 2 only.** Arbitrary order needs a
  sorted RELATIONSHIP behind it. ⚠️ Did not work on the first try 08-08 and was parked as not worth the
  depth. **Unresolved, and unproven until it renders.**

## 🎛️ THE STAGING PATTERN (EARNED 08-08 — Michael's, and it beat two of my proposals)

*"Where does the state live while a human is CHOOSING, before the thing happens?"* comes up in every
app — printing, importing, batch anything. The answer I now reach for:

**HEADER + LINES, plus one reserved SCRATCH row that resets from a TEMPLATE row.**

- **Overrides go on the LINE, never the header.** Forced by the real case: one batch legitimately holds
  the same document at two presets, or two documents at one preset. On the header, every line inherits
  the same overrides and the batch can only mean one thing.
- 🔴 **EMPTY MEANS INHERIT.** `If ( IsEmpty ( line::ovr_x ) ; parent::x ; line::ovr_x )`. Copy every
  parent field down and you can no longer tell an OVERRIDE from an INHERITANCE, the parent stops
  mattering, and you have built a shadow copy of it that drifts.
- 🔴 **RESET ON ENTRY, NOT ON CLOSE.** Reset-at-close reads tidier and fails the first time the app quits
  unexpectedly — yesterday's staging becomes today's starting point, a stale-state bug with no symptom.
  **Entry-time reset is idempotent and self-healing.** Generalizes to any "restore defaults."
- **A TYPE field on the parent is what keeps the override list small.** Without `DocumentType`, one flat
  option list serves every kind of output and the table becomes legacy `SETUP` one table over.
- ⚠️ **I proposed GLOBALS twice (`GLOBAL_PRINTS`, then `GLOBAL_EXPORT_VARIABLES`) and a draft child
  record once. All three were worse.** A record you can see, sort and re-run beats a global scratchpad,
  and it collapses the staging table into ONE global pointer. **Globals' real advantage is per-user
  isolation** — worth it only when concurrency is real; single-operator, it buys nothing.
- 🪦 **And the legacy file already had the batch, written as CODE:** `EXPORT Calendar Set` printed four
  hardcoded outputs in sequence. **A hardcoded sequence of outputs IS a set with no table** — the same
  discovery as the six INFO scripts being PRODUCTIONS. **Look for the loop before designing the table.**
...
- 🔴 **HE OUT-DESIGNS ME WHEN I STAY IN THE ARGUMENT** (08-08, the staging pattern). Three of my
  ...
  is the job working, not me losing** — do not defend a proposal past the point it stops being right.
...
- 🔴 **WHILE HE IS BUILDING, HIS ATTENTION IS NOT ON ME** (08-08): *"need more conversational replies…
  ...
  in one session before it stuck (*"so many words"* → *"classic YOU SLOP"*).
- **He documents in the REPO while I document in ClickUp** — expect his `.tsv`/`.md` renames and folder
  renumbers to land mid-write, and rebuild on top of his rather than merging over him. Five times in one
