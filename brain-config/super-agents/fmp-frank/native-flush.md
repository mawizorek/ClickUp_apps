...
- **2026-08-01 — DUMP of 2026-08-01 ~11:40 AM ET consolidated and cleared** by Memory Maggie (first live run of `hooks/native-flush-consolidation.md`). Two source pages (`filemaker_git_migration.md`, `super_agent_fleet.md`), ~10KB verbatim. Placed to `memory.md`: the stale-fleet-cache precedent, the retired-roster pointer correction, the Model A lineage line, the green-light authorization rule. Rotated to `memory/archive/hml-llc-fmp19-build-detail.md` for cap. Dropped as owned elsewhere: Model A conversion narrative (→ `activity-log.md` + `preferences.md`), the flush-pipeline design ruling (→ the hook itself), the OMR capacity crisis (→ Maggie's own ledger), the mobile-tables preference (→ already queued as `OMR-20260731-4`), runbooks A/B (→ procedure, not memory). **Fleet-fact rot reported to the Steward and fixed the same pass:** the dump named Corey as Fleet Steward and `superagents.json` as canonical.

## Dump zone (newest on top)

*(bare — no unconsolidated delta)*
