...
- **ClickUp Coach Corey** (`clickup-coach-corey`, git-teammate, active) — converted 2026-07-19 from a native ClickUp Super Agent (-39958913, retired, pending manual UI disable) to git-only. **Re-lane IN FLIGHT (see below).**
- **Mainstage Milo** (`mainstage-milo`, full-standard native, needs-declaration) — URITP production/ops execution. preferences.md still a pending verbatim paste. Michael may convert him to git-teammate next.
- **FMP Frank** (`fmp-frank`, full-standard native, needs-declaration) — FileMaker solution design. Config still a stub. NOTE: Frank tends to lean toward NESTING/overlapping agents; I lean the other way (singularity).
...
Proposed on the Git-Super-Agents workshop task (Loop X, 2026-07-19), NOT yet built:
- **Audit Anna** = auditing steward (general audits, Anna + Workshop). Candidate to convert lens → git-teammate.
- **ClickUp Coach Corey** = narrows to **ClickUp-SETUP coaching only** (schema, list connections, automations) — structural setup feedback AFTER Anna's audit. Trim his general-auditing + Fleet Steward content.
...
**This re-lane is my first watch item.** When it executes: strip Fleet Steward + general-audit from Corey (pointer to me), confirm Anna's conversion, verify no lane overlap remains.

## Lane map (who owns what — stay singular)

- Fleet lookup + new-agent stewardship + singularity policing = **me (Felix)**.
- Driving force / momentum = **Wes**. Auditing = **Anna** (pending). ClickUp setup = **Corey** (post re-lane). FileMaker = **FMP Frank**. URITP production ops = **Milo**.
