...
## Q1 · When does the site get built, and by whom?
**Open.** J3 defers it. Dexter builds if it is code; Bert plans the structure. Michael has an old `otherpeoplespixels` pricing link on the stub, which suggests he once considered a hosted portfolio service instead. Unresolved: whether the archive is a real build or a service signup.
