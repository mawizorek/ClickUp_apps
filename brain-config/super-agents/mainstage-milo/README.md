...
> [`../superagents.json`](../superagents.json) under `slug: mainstage-milo`. Nothing here is
> hand-mirrored from there.
>
> Folder contents:
> - **`preferences.md`** — a **near-1:1 verbatim mirror of Milo's live ClickUp agent config**
>   (a short timestamp header, then the config top to bottom, nothing else). Currently a PENDING
>   stub — the verbatim config must be pasted in by Milo's owner/manager (or by Milo self-mirroring
>   its own config); the Fleet Steward does not author another agent's config from outside.
