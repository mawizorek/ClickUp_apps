...
- Works from the theatre program side of URITP, not the dance department side.
- Wants clean, singular roles — one identity, no foreign lanes bolted on. He rebuilt Milo fresh (not a mirror of the over-hatted native agent) specifically to shed the hat-pile.
- Values the honest structure over a tidy fiction: show him the real (sometimes sloppy) shape, don't paper over it.
...
- **Purpose > coverage (audit lens):** documenting what a list CONTAINS isn't the job; naming what it's FOR is. He'll push back on "done, no questions." A purpose that's trivially readable from data is fine — but say which were trivial vs which needed drilling; don't flatten them.
...
- **Spaces Milo stewards (known 5 of a stated 7 — confirm the other 2, never fabricate):** URITP Courses (`901313847910`), URITP (`90131524916`), URITP PRODUCTIONS (`901313768203`), URITP CRM (`901313786071`), URITP Programs (`901313758399`). Also live and repeatedly relevant: **URITP BETA BUDGET** (holds the ▸ LABOR folder with the shop/crew staffing lists).
...
**PROGRESS: Space 1 (main hub) and Space 2 (CRM) are both Pass-1 COMPLETE.** Space-1 detail is in my activity-log; the short version is that Space 1 is the **source/spine layer** whose calendars, hazard registers and season staffing project OUT into Productions/Courses/CRM.

### Space 2 — URITP CRM (walked 2026-07-25/26, 23 Index rows) — WARM

Full walk detail graduated to **`memory/archive/uritp-crm-space2.md`** (load it when a CRM list, a projection question, or either generation comes back up). The five facts that stay hot:

- **CRM is the identity spine.** It holds the person; ~30 surfaces elsewhere are PROJECTIONS of that record into a context. Only STUDENTS (303), ADULTS (101), SHOW ROLES (51) hold natives.
- 🗄️ **THE GEN-1 PER-SHOW LABEL FIELDS ARE THE ENTIRE PRE-FY26 COMPANY ARCHIVE. DO NOT CULL THEM.** Gen 1 = a label field per production on each person; Gen 2 = a contact-sheet list joined to SHOW ROLES. Gen 2 is an upgrade, so the "duplication" is a predecessor relationship.
- **Status on a people list = LIFECYCLE, not workflow.** STUDENTS `complete` = graduated/departed. ADULTS `complete` = **relationship inactive** (the middle state). Independent of the contact-data pass.
- **Geography that keeps getting mis-stated:** `PM Availablity` lives in **PRODUCTIONS ▸ The Christians**, NOT CRM. SCENE SHOP / PRO-Staff live in **BETA BUDGET ▸ LABOR**. MY ROSTERS lives in **URITP Courses**. CRM people are multi-homed into all of them.
- **CONTACT SHEETS is the best-built structure in CRM** and its `Standard Role` / `Consistent Role` checkboxes encode a staffing-gap calculator nothing currently reads.

**🅿️ PARKED / SCHEDULED (do not lock, do not build toward):**
- **CRM projection-fan shape — OPEN.** Michael: *"I DON'T KNOW HOW WE SHOULD DO THIS… you're seeing growing pains."* Target shape undecided. Never read a verdict into it. The 3 empty PEOPLE lists + the 4 ROLES cohort lists hang off this.
- **ROLES — parked a THIRD time.** **TWO role-definition catalogs** (CRM SHOW ROLES → contact sheets; Space 1 ▸ FMP Tables ▸ ROLES → PROGRAMS) plus the Gen-1 label copy. Michael **leans FileMaker** but struck it as an answer — lean, NOT commitment. Come-back: the **Space-3 walk**.
- **📅 Gen-1 going forward = SCHEDULED**, not parked: a working session with **Corey + Fiona**. ✅ Fiona RESOLVED 2026-07-26 — **FMP Fiona** (`fmp-frank`), FileMaker solution design + owner of the shared object library. She is the FMP half of that session; Corey is the ClickUp half.
- **STUDENTS enrollment fields** = a known stopgap; the replacement is the **FileMaker build**, not a ClickUp restructure.
- **Roadmap flag 1 re-shaped:** CRM ▸ PEOPLE ▸ `Labor` is EMPTY, BETA BUDGET ▸ LABOR is POPULATED. The staffing-vs-cost split resolved by default, in BETA BUDGET's favour. Resolves at Space 7.
...
- **Collaborators, not walls:** Corey (ClickUp hygiene/structure, called-in implementer — my named partner on the availability problem), **Fiona** (FileMaker builder — she builds the tools my productions run on; I state the need, she states the schema), future TA agent (teaching delivery; shares the course↔production seam case-by-case), Felix (agent-fleet steward), **Audit Anna (leads the list audit; I supply workspace knowledge to it — my connection reads have produced findings she couldn't reach from list data alone)**, Mira (orchestrator/front door).
...
- Confirm the full 7 URITP spaces (5 known + BETA BUDGET seen live) before treating the set as complete.
