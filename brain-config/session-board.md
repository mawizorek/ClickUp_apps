...
|---|---|---|---|---|
| Dev Dexter | Print fills · markdown tables · Aug 30 | `fix/markdown-table-print-fill` (doc-render-engine) · `board/dexter-table-fill` (ClickUp_apps) | Michael printed Todd Union Keys: the plain pipe table prints as a solid parchment slab. 🔴 **THIRD INSTANCE of one documented mechanism** — print.css already killed the content-column ground (08-19) and the four `.dr-data` grounds (08-28), and its own comment predicted the recurrence in writing. **`table:not([class])` was never covered.** Fix: fills off on the unclassed markdown table only (the TSV table carries a class ON PURPOSE and keeps its own rules). ⚠️ Also means his print dialog has **Background graphics ON** — that selector is not on print.css's `print-color-adjust: exact` list, so with it off the fill would already have dropped. 🔴 **PLACEMENT FORCED BY BYTES:** print.css is **22,232 B / 296 B of headroom** and cannot take the block; a new print sheet needs `assets.py` (32,684 B, past the WRITE cap); a CSS sidecar would be a net-new convention I will not invent unasked. So the rule lands in **`print-ink.css` (9,336 B)** — the only registered print sheet with room, whose subject is colour, and whose 🚫 "next colour opinion needs its own ruling" gate is satisfied by Michael's ask. **A one-line cross-pointer goes in print.css so grounds never have two silent claimants.** | **doc-render-engine**: `assets/print-ink.css` (rule + argument) · `assets/print.css` (ONE pointer line, ~110 B, keeps it under the 22,528 ceiling — measured, not estimated). 🚫 NOT `assets/data.css` (its print rules deliberately live in the print group) · NOT `flow.css` (23,163 B, past ceiling) · NOT `assets.py`/`mkdocs.yml`/`qr.py` (past write cap). 🚫 No `uritp-safety` write — every markdown table on every site fixes itself on merge. |
