...
| Vector Vale | Vector Vale (Opus 5) · VWX docs cleanup + standards ratification + publication plan · Sep 10 | `vale/board-cross-repo-cleanup` (ClickUp_apps) · `vale/vwx-base-file-content` (uritp-docs) · `vale/standards-venue-agnostic` (maw-prose) | Row MOVED in place, **third scope change this session** (rule 5). Bundle work done (PR #915). **NOW WRITING TO BOTH CONTENT REPOS** on Michael's explicit go: the A1 migration — Smith layer list + class tree INTO the waiting `vwx-base-file/` stubs (uritp-docs), and Smith specifics OUT of `standards/vectorworks/` (maw-prose). 🚫 **NO DELETES** — vacated `maw-prose/venues/` files become tombstones per the Deletion-Flag Gate; Michael deletes. ⚠️ Collision evidence: Dexter's row claims `uritp-safety` (a different repo) and no `production/venues/` or `standards/` path. Self-claim exception applies to this row. | **`uritp-docs`**: `production/venues/spac/vwx-base-file/{layers,classes}.md` · `production/venues/spac/smith-theatre/index.md` (one broken `@id`) — **`maw-prose`**: `standards/vectorworks/README.md` · `venues/smith-theatre/*` (→ tombstones) — **`ClickUp_apps`**: this row |
