...
_⭐ **THE DURABLE LINE FROM ALL SEVEN: every collision was caught by a READ, none by a CHECK.** Our mechanisms all try to answer *"may I write?"* in advance. The only thing that has ever worked is asking *"what is true right now?"* at the last possible moment — path-filtered `list_commits --since today` immediately before the write, and treating a rejected write as information rather than an obstacle._
