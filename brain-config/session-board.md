...
|---|---|---|---|---|
| Dev Dexter | Embed ANY ClickUp view in a doc-renderer page · Aug 30 | `feature/directive-align` (doc-render-engine) · `board/dexter-align-parser` (ClickUp_apps) | Teaching `!!! form` and `!!! view` the trailing `key=value` option parser, so his `align=center` stops silently killing three directives. Shared `align` parser goes in **`util.py`** returning `(value, problems)` — pure, so the caller reports and `util` keeps its no-framework promise. 🔴 **THE HONEST HALF: an embed frame is `width: 100%`, so alignment cannot move it** — align.css's own rasterised measurement says only margins move a sized box and a full-width box has zero slack. What aligns is the FURNITURE (summary label, reload button, fallback link). Documented, not oversold. 🅿️ `qr.py` CANNOT adopt the shared parser this pass — 29,915 B, at the `create_or_update_file` write cap; so align parsing lives in two places and that debt is recorded rather than hidden. | **doc-render-engine**: `docrender/util.py` (append one pure helper — ⚠️ this file was CLOBBERED once from a stale read, re-read at HEAD `00c12f17`) · `docrender/forms.py` (regex + class only; 🔴 22,059 B against its own 22,528 ceiling, so prose goes to the sidecar) · `docrender/views.py` · `docrender/forms-dl.md` + `views-dl.md` (the reasoning) · `assets/align.css` (11,005 B, owns alignment, has room). 🚫 NOT `assets/flow.css` (23,163 B, past ceiling) · NOT `qr.py` · NOT `mkdocs.yml`. 🚫 No `uritp-safety` write — his page starts working the moment this merges, no content edit needed. |
