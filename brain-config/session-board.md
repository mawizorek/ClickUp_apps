...
| Fleet Felix (Opus 5) | Session task titled `Fleet Felix (Opus 5) · Build Realty Riley — real-estate business owner + Milo's counterpart · Aug 1` (Agent Activity Board) | **BUILT REALTY RILEY, teammate #13** — real-estate business owner, the structural counterpart to Mainstage Milo. Bundle merged in **PR #660**; Agent Index row live; AI Toolkit trigger row added. ⚠️ **DELIBERATELY SPLIT from the parallel URITP department-head Workshop** on the Fleet Build Queue standing thread (Michael: *"cut Riley now, let the URITP room finish separately"*) — two agent-build conversations running at once, two records, no shared task. **Collision-checked 08-01 15:00 ET: branch `felix/build-realty-riley` created clean (no "Reference already exists"), and every repo path is NET-NEW under a slug nobody else can be holding.** | **`ClickUp_apps`:** `super-agents/realty-riley/**` (all new, RELEASED at merge), `session-board.md` |
...
_⚠️ **Never hand-type a ClickUp ID into a durable file.** Three recurrences, the third of which landed on `main`. **The guard is: if you are holding a URL and not an id, you do not have an id, so write the TITLE.** Every other row on this board names its session by title for exactly that reason. ⚠️ **08-01: FOURTH recurrence, caught pre-merge.** Riley's `activity-log.md` shipped a draft carrying a task URL the session did not actually hold; fixed in `f56cc8a` by naming the task by TITLE. **Four instances means the guard is not being reached at write time** — it lives in a board footnote and in no checklist, which is precisely the B19 shape.
...
_🧭 **THE STANDING GUARD HOLDS: do not invent an agent to fill an empty queue.** ⚠️ **CORRECTED 08-01** — this note used to read "12 teammates, zero unbuilt, queue is EMPTY (Clark cancelled 07-28)" and it is **13 as of today** (Realty Riley, PR #660). **Riley was not invented to fill the queue; Michael walked in with the need and the lane was genuinely unowned** — Fiona knew the HML_LLC tables and nobody knew why the loan mattered. The guard is about manufacturing demand, not about refusing it. ⚠️ **A second agent conversation is live in parallel** — URITP department heads, on the Fleet Build Queue standing thread with Mira's Workshop, deliberately kept separate from this build. ⚠️ **Do not put a count in this note again.** Filter the 🤖 Agent Index by `Class` and count the rows; that is the only number that cannot rot. The standing thread's DESCRIPTION also still says "First move: Catch Up Clark" — still wrong, still unfixed, trust the checklist and DL J14._
