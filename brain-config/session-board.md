...
|---|---|---|---|---|
| Dev Dexter | Write-cap recheck — the two "unsplittable" files · Aug 22 | `board/dexter-cap-recheck` (ClickUp_apps) | Seated to split `on-track/data.json` by month and `routines/job-market-refresh.md` under the write cap. **Both premises FAILED verification at HEAD — see the PR.** `job-market-refresh.md` (34,668 B) read back WHOLE; `on-track/data.json` (34,783 B) was written and read back whole earlier today. **Holding both splits pending Michael's ruling; correcting the docs that assert the cap instead.** ⚠️ Prior row (Aug 18, `board/dexter-urllinks`) was EXPIRED under rule 8 — no commit to any claimed path in 4 days, all its work merged. Moved in place per rule 5 rather than retired, because it is the same agent and the same standing session lineage. | **ClickUp_apps**: this file · `routines/schedule.md`. 🅿️ **`VERSIONS.md` is OWED, deliberately NOT in this pass** — its `on-track` row claims `data 2026-07-23c` against `2026-08-22` at HEAD, but the ledger carries a standing TRIM OWED and its own rule that an edit which adds must trim in the same commit. That makes the row correction its own act, not a ride-along. 🚫 Not touching `on-track/source/*`, `app.js` or `render.js` — no engine change without the ruling. |
