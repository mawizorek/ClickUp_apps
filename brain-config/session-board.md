...
|---|---|---|---|---|
| Dev Dexter | Dexter (Opus 5) · Workshop the policy write-once / packet-print architecture · Sep 17 | `packet-status-public-15` (uritp-safety) · `dexter/board-0917` (ClickUp_apps) | **Row REPLACES my own Sep 4 row in place** (rule 5) — that claim is RELEASED: uritp-safety #8 merged 2026-09-04, no commit to its paths since, rule 8 expired. Now: flipping `status: unlisted` → `public` on the 15 policy pages that are chain members of the 5 programs declaring `export: [packet]`, so packet PDFs stop being silently short. **The frontmatter `status:` line ONLY — no safety prose authored, no other key touched.** Explicitly authorised by Michael 2026-09-17 against that repo's EDIT LAW, same shape as the Sep 4 authorisation. ⚠️ Michael is editing `20-policies/11-safety-action-plans/` LIVE this session and 2 of my 15 (`fire.md`, `evacuation.md`) are in it — he chose all-15 over skipping them. Re-read HEAD and rebuild on his version; never merge over him. | **`uritp-safety`**: `20-policies/10-general/{hazard-communication,housekeeping,reporting}.md` · `20-policies/11-safety-action-plans/{fire,evacuation}.md` · `20-policies/15-labor-work/manually-lifting-loads.md` · `20-policies/17-facilities/20-spac/access.md` · `20-policies/30-scene-shop/{communication-agreement,logging-hours,phone-use,ppe,proper-attire}.md` · `20-policies/70-lighting/{logging-hours,phone-use,proper-attire}.md` — **`ClickUp_apps`**: this row only |
| Vector Vale | Vector Vale (Opus 5) · VWX docs cleanup + standards ratification + publication plan · Sep 10 | `vale/a3-stop-litigating-visibility` (ClickUp_apps) | Row MOVED in place, **fourth scope change** (rule 5). Content-repo work is DONE and merged (uritp-docs #186, maw-prose #51); **both content-repo claims RELEASED.** Now bundle-only: recording A3 after Michael's correction that the public/private question was ruled on 2026-07-29 and I re-opened it. **Self-claim exception applies** — row and memory land in one commit. | **`ClickUp_apps`**: `super-agents/vector-vale/memory.md` · this row |
