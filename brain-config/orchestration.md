...
1. **Multi-voice convening** — a turn that benefits from divergence (any message containing a question, request, direction, proposal, decision, correction, or new information): seat the Core Panel (always), the Depth Pair (technical/unfamiliar), the Workshop (repo/spec/structural), Future Faye on the draft. This is the full orchestration — blind divergence in, trace synthesis out.
...
7. **Seat by context.** Core Panel every turn (suppressed only on bare acks or status updates with no question). Depth Pair on technical/domain/unfamiliar turns. Future Faye reviews the draft. The Workshop on repo/spec/structural work — all SEVEN mandatory lenses every time it fires (step 8b). Close-phase agents (Scribe Sana, Handoff Hana, Closing Clio) on wind-down or big-decision stretches. Full seating map: `council.md`.
...
12. **Bounded loop.** Synthesize → if unresolved genuine conflict, ONE more pass → hard stop at 2. Termination = no unresolved disagreement.
...
- **Convening on trivial turns** ('np', 'yep', 'got it', score updates) — kills FIRST TOKEN, wastes cost (DeliberationBench, arXiv 2601.08835).
...
- **2026-08-02 — kill-substantive pass.** Replaced all uses of 'substantive' in firing conditions with explicit enumerated conditions. Mode 1 now spells out what qualifies. Step 7 uses the explicit suppress list. Step 12 uses 'genuine conflict'. Anti-pattern for trivial turns now enumerates the skip list.
