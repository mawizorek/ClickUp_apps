...
~~The registry↔index mirror pair is the ONE sanctioned duplication in this system, and it only survives because we reconcile both sides in the same session, every time.~~ **CORRECTED 2026-07-25: there are now ZERO sanctioned duplications.** `registry.json` was tombstoned earlier the same day (`roster.json` is the single documented source for agents) and the Mirror-Pair Sync Mandate was retired from the AI Toolkit index. Every duplication in this system is now a defect. The struck sentence is kept because it was cited as precedent for tolerating mirrors, and that precedent is gone.
