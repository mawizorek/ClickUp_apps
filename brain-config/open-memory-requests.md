...
#### ✚ ADDENDUM (2026-07-26, Brain — requested by Michael to strengthen the entry before the drain)

**A1 · Proposed exact wording, so the drain is transcription and not authorship.** Land this as ONE line inside the existing LOAD-THEN-THINK step-zero block (not as a new section, not as a Workflow Default):

> **SPINE FIRST, every substantive reply:** the live chat is volatile and is NEVER the record. Post the one-line spine entry to the Agent Activity Board channel (https://app.clickup.com/36074068/chat/r/6-901327879922-8), threaded under this session's header post, BEFORE sending the reply. Order: first token → tool work → spine line → reply. Re-resolve the header from the channel, never a cached ID. Failed write never blocks the reply: mark `⚠️ spine write failed` and backfill. Spec: `gates/session-transcript-gate.md`.

Maggie may compress the prose, but three elements are load-bearing and must survive any trim: **(a)** the channel URL, **(b)** the word BEFORE (the ordering is the rule; "log each reply" without it degrades to the afterthought this replaced), **(c)** never-block. Drop any of the three and the line stops working.

**A2 · Edit Guard clearance, pre-argued.** Step 0 (Preference-Save Routing) resolves at door 1: Michael explicitly directed brain memory ("could we just drop a line in real memory that says…"), so the memory write is authorized rather than defaulted. Step 3 (Firing-Reliability) passes on the strictest reading — this fires on every single response regardless of context, which is the ONE category memory exists for. Step 6 (No Per-Tool Memory Lines) needs a deliberate ruling: this names a specific surface, which resembles the per-tool line the guard forbids. **Argued distinction:** the ban exists to stop per-TOOL activation lines that should route through the index roster. This is not a tool activation, it is an **unconditional output-ordering rule** — the same class as the load-then-think directive that the guard already carves out as the one allowed line. If Maggie reads it as a Step 6 violation, the fallback is a compressed pointer ("spine-first on every reply → `gates/session-transcript-gate.md`") which keeps (b) and (c) as the cost of losing (a).

**A3 · Why memory is the ONLY enforcement layer for this one, stated plainly.** The gate is merged and correct, and a merged gate is inert until something routes to it. Every other spine rule is self-enforcing by construction: arming is a side effect of posting the header, never-block is a failure path, format is checked at write time. **Ordering alone has no mechanism.** It is unobservable after the fact (a line and a reply in the same minute are indistinguishable from a line written after), so the close-time reply/line count will pass a session that logged in the wrong order every time. There is no detector to build. That leaves exactly one enforcement surface: the must-fire memory layer. This is the rare case where "put it in memory" is not the lazy answer, it is the only answer.

**A4 · Precedent worth recording, since precedent is why Maggie graduated.** This is the first entry in the queue whose *own subject matter* is the reason the queue keeps failing. `OMR-20260722-1` (no direct memory writes) has sat unlanded since 07-22 and its exact prohibited behavior recurred on 07-26. `OMR-20260717-1` and `-2` have sat since 07-17. **An unlanded behavioral rule does not fail safe, it reproduces the mistake it was written to prevent.** Recommend the drain prioritize by that criterion — recurrence risk, not arrival order.

**A5 · Deletion note.** If Maggie denies brain-memory placement, the entry must NOT be silently closed as "covered by the gate." The gate is already merged; coverage was never the question. A denial needs a named alternative enforcement surface, or the rule ships unenforced and this whole session's work decays into the `Follow Up` tag's grave.
