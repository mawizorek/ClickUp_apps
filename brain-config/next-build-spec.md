...
>
> 🔁 **AND THEN THE CORRECTION ITSELF ROTTED (2026-08-01).** The 07-25 fix below repointed a dead manifest at `roster.json` and quoted its ~12KB cap as live law. **`roster.json` was retired five days later**, so for two days this entry corrected one dead pointer with another — inside the very block that exists to teach that authoritative text can be wrong. Layered in place rather than rewritten, because **the third strike is the actual lesson: a correction inherits the shelf life of whatever it points AT.**

- **~~registry.json is a generated manifest, do NOT hand-edit~~ → RETIRED 2026-07-25.** `registry.json` was a 2026-07-04 bootstrap manifest that predated git-teammates by eleven days and eventually grew unreadable; it is now a tombstone stub. ~~**`brain-config/super-agents/roster.json` is THE single agent roster** — one flat list, hand-edited by design, capped at ~12KB precisely to stay hand-editable.~~ **→ ALSO RETIRED 2026-07-30** (with `roster.html`; `superagents.json` before them — **four retired manifests**). **The fleet record is the 🤖 Agent Index ClickUp list** (`901328043244`), one task per agent. **A list has fields; fields refuse essays** — which is why the size cap died with the file rather than being re-imposed. The old "never hand-edit the generated manifest" caution stays inverted: the Index is edited by hand, by design. The `shortcut | launchPrompt` fields belong on the tool profile's front-matter, which stays canonical.
...
- **Never write a size, count or status you have not read back.** On 2026-08-01, nine commit messages in one session stated a byte count that was wrong on arrival — including one claiming a trim on a file it grew 26%, and one wrong by 50% inside the sentence documenting the pattern. **Read the returned value, then write the claim.**
