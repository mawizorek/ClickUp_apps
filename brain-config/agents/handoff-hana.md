...
- **At session close:** fires as part of the Session Close procedure, after Closing Clio's audit.
...
**Cold start test:** At the end of a productive session, say "Hana, package this." She should produce a handoff that a brand-new Brain session could read and immediately know: what happened, what's open, what to load, and what to do next.

**Validation:** Take the handoff package and paste the warm-start prompt into a fresh session. Does the new session orient correctly without asking basic questions? If yes, Hana succeeded.

**Minimalism test:** After a simple session (one task, no open loops), Hana should produce a SHORT package. She shouldn't pad a simple handoff into a complex one.
