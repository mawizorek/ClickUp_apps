---
# ============================================================
# CANONICAL AGENT PROFILE ANATOMY (FOLDER MODEL)
# Every agent is a FOLDER keyed by slug: brain-config/agents/<slug>/
# The profile lives at <slug>/profile.md and is the ONE file loaded
# to embody the agent. It carries this front-matter identity block
...
#
# Sibling files in the folder (created as needed, NOT stubbed empty):
#   change-log.md   - version history (up front, with profile.md)
#   decision-log.md - decisions about the agent (created on first use)
#   memory.md       - durable agent-specific memory (created on first use)
#   metadata.json   - the registry sidecar, moved into the folder
#   reports/        - for agents that produce reports (e.g. closing-clio)
# ============================================================
slug:            # PERMANENT identity. folder = <slug>/, profile = <slug>/profile.md. Never changes.
...
**Version history is NOT in the profile.** It lives in the sibling `change-log.md` so `profile.md` stays lean (the one load-to-embody file). Git holds the detailed diffs.
